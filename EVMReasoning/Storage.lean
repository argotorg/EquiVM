...
theorem codeOwnerStorageWord_initState {cA gh bl σ σ₀ A I} {g : Sat256}
    (slot : UInt256) :
    Storage.EVM.storageLoad (initState cA gh bl σ σ₀ g A I) I.codeOwner slot =
      codeOwnerStorageWord I σ slot := by
  simp [Storage.EVM.storageLoad, initState, State.lookupAccount, Account.lookupStorage,
...
theorem storageLocLoad_uint256 (evm : EVM.State) (slot : UInt256) :
    storageLocLoad evm (uint256Loc slot) =
      .int (Int.ofNat (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot).toNat) := by
  have htake :
      (EVM.Word.toBytesLEWithSizeProof
          (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)).1.extract 0 (32 : Fin 33).val =
        (EVM.Word.toBytesLEWithSizeProof
          (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)).1 := by
    rw [List.extract_eq_take_drop, List.drop_zero]
    exact List.take_of_length_le (by
      rw [(EVM.Word.toBytesLEWithSizeProof
        (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)).2]
      norm_num)
...
theorem storageLocStore_uint256 (evm : EVM.State) (slot val : UInt256) :
    storageLocStore evm (uint256Loc slot) (.int (Int.ofNat val.toNat)) =
      some (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot val) := by
...
theorem storageLocLoad_bytes32 (evm : EVM.State) (slot : UInt256) :
    storageLocLoad evm (bytes32Loc slot) =
      .fixedBytes ⟨31, by decide⟩
        (EVM.Word.toBytesBE (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)) := by
  have htake :
      (EVM.Word.toBytesLEWithSizeProof
          (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)).1.extract 0 (32 : Fin 33).val =
        (EVM.Word.toBytesLEWithSizeProof
...
theorem storageLocLoad_uint_offset0 (evm : EVM.State) (slot : UInt256)
    ...
    storageLocLoad evm
      ...
      .int (Int.ofNat (UInt256.land
        (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)
  ...
  simpa [Nat.sub_zero] using
    fromBytes'_take_wordLE_land_mask
      (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) size.val hbits
...
theorem storageLocLoad_uint_offset (evm : EVM.State) (slot : UInt256)
    ...
    storageLocLoad evm
        { slot := slot, offset := offset, size := size, hbound := hbound,
          type := .int (.uint width) } =
      .int (Int.ofNat (UInt256.land
        (UInt256.div (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)
...
theorem checkBytesPacked_of_storageLoad_land_one_zero {evm : EVM.State}
    {base header : UInt256}
    (hload : Storage.EVM.storageLoad evm evm.executionEnv.codeOwner base = header)
    (hflag : UInt256.land header ⟨1⟩ = ⟨0⟩) :
...
theorem storageLocLoad_address_offset0 (evm : EVM.State) (slot : UInt256) :
    storageLocLoad evm (addressOffset0Loc slot) =
      .address (AccountAddress.ofNat
        (UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)
...
theorem storageLocStore_address_offset0 (evm : EVM.State)
    ...
    storageLocStore evm (addressOffset0Loc slot)
      ...
      some (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot
        (setAddressOffset0Word
          (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) addr)) := by
  ...
  show fromBytes'
      (List.take (0 : Fin 32).val _ ++ List.take (20 : Fin 33).val _
        ...
        (setAddressOffset0Word
          (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) addr).toNat
...
theorem storageLocLoad_address_offset1 (evm : EVM.State) (slot : UInt256)
    ...
    storageLocLoad evm
      ...
      .address (AccountAddress.ofNat
        (UInt256.land
          (UInt256.div (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) ⟨256⟩)
          solcAddrMask).toNat) := by
  unfold storageLocLoad wordToElem
...
theorem storageLocLoad_bool_offset0 (evm : EVM.State) (slot : UInt256) :
    storageLocLoad evm (boolOffset0Loc slot) =
      wordToElem .bool
        (UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) ⟨255⟩) := by
  unfold storageLocLoad boolOffset0Loc
  simp only [Fin.val_zero, Nat.zero_add]
  congr
  change fromBytes' ((EVM.Word.toBytesLEWithSizeProof
      (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)).1.take 1) = _
  rw [fromBytes'_take_wordLE_land_mask (n := 1) _ (by decide)]
  rfl

theorem storageLocLoad_bool_offset0_false (evm : EVM.State) (slot : UInt256)
    (hzero : UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) ⟨255⟩ =
      ⟨0⟩) :
...
theorem storageLocLoad_bool_offset0_true (evm : EVM.State) (slot : UInt256)
    (hnz : UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) ⟨255⟩ ≠
      ⟨0⟩) :
    storageLocLoad evm (boolOffset0Loc slot) = .bool true := by
  rw [storageLocLoad_bool_offset0 evm slot]
  have hbeq :
      ((UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) ⟨255⟩).val == 0) =
        false := by
...
theorem storageLocStore_bool_word_offset0 (evm : EVM.State) (slot word : UInt256) :
    storageLocStore evm (boolOffset0Loc slot) (wordToElem .bool word) =
      some (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot
        (setBoolOffset0Word (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot) word)) := by
...
theorem storageLoad_accountMapEquiv {evm1 evm2 : EVM.State}
    ...
    Storage.EVM.storageLoad evm1 addr slot = Storage.EVM.storageLoad evm2 addr slot := by
  simp [Storage.EVM.storageLoad, State.lookupAccount, Account.lookupStorage]
...
theorem initState_codeOwner_storageLoad_ne_of_accountMapEquiv
    ...
    Storage.EVM.storageLoad (initState cA gh bl σ_solm σ₀ g A I)
  ...
  simpa [initState, Storage.EVM.storageLoad, State.lookupAccount, Account.lookupStorage] using hword
...
theorem storageStore_accountMapEquiv {evm1 evm2 : EVM.State}
    ...
    accountMapEquiv (Storage.EVM.storageStore evm1 addr slot val).accountMap
      (Storage.EVM.storageStore evm2 addr slot val).accountMap := by
...
theorem storageStore_executionEnv (evm : EVM.State) (addr : AccountAddress)
    (slot val : UInt256) :
    (Storage.EVM.storageStore evm addr slot val).executionEnv = evm.executionEnv := by
...
theorem storageStore_absent (evm : EVM.State) (addr : AccountAddress)
  ...
  simp [Storage.EVM.storageStore, State.lookupAccount, hmissing, Option.option]
...
theorem storageLoad_storageStore_same_present (evm : EVM.State) (addr : AccountAddress)
    ...
    Storage.EVM.storageLoad (Storage.EVM.storageStore evm addr slot val) addr slot = val := by
  unfold Storage.EVM.storageLoad Storage.EVM.storageStore State.lookupAccount
  rw [hacc]
  simp only [Option.option]
...
theorem storageLocStore_address_offset1_after_bool_true (evm : EVM.State)
    ...
    {hbound : (1 : Fin 32).val + (20 : Fin 33).val - 1 < 32} :
    storageLocStore
        (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot
          (UInt256.lor
            (UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)
              (UInt256.lnot ⟨255⟩)) ⟨1⟩))
        { slot := slot, offset := 1, size := 20, hbound := hbound, type := .address }
        (.address (AccountAddress.ofNat val.toNat)) =
      some (Storage.EVM.storageStore
        (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot
          (UInt256.lor
            (UInt256.land (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot)
              (UInt256.lnot ⟨255⟩)) ⟨1⟩))
        ...
        (UInt256.lor ⟨1⟩
          (UInt256.lor
            ...
            (UInt256.land
              ...
              (Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot))))) := by
  ...
  let old := Storage.EVM.storageLoad evm evm.executionEnv.codeOwner slot
  let boolWord := UInt256.lor (UInt256.land old (UInt256.lnot ⟨255⟩)) ⟨1⟩
  have hload :
      Storage.EVM.storageLoad
          (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot boolWord)
  ...
  apply congrArg some
  apply congrArg
    (fun w => Storage.EVM.storageStore
      (Storage.EVM.storageStore evm evm.executionEnv.codeOwner slot boolWord)
...
theorem storageLoad_storageStore_ne (evm : EVM.State) (addr : AccountAddress)
    {readSlot writeSlot val : UInt256} (hne : readSlot ≠ writeSlot) :
    Storage.EVM.storageLoad (Storage.EVM.storageStore evm addr writeSlot val) addr readSlot =
      Storage.EVM.storageLoad evm addr readSlot := by
  simp only [Storage.EVM.storageLoad, Storage.EVM.storageStore, State.lookupAccount]
...
theorem storageLoad {evm₁ evm₂ : EVM.State} (h : EVMStateEquiv evm₁ evm₂)
    ...
    Storage.EVM.storageLoad evm₁ addr₁ slot = Storage.EVM.storageLoad evm₂ addr₂ slot := by
  subst addr₂
...
theorem storageStore {evm₁ evm₂ : EVM.State} (h : EVMStateEquiv evm₁ evm₂)
    ...
    EVMStateEquiv (Storage.EVM.storageStore evm₁ addr₁ slot val₁)
      (Storage.EVM.storageStore evm₂ addr₂ slot val₂) := by
  subst addr₂
  subst val₂
  refine ⟨?_, ?_, ?_⟩
...
theorem storageStore_codeOwner {evm₁ evm₂ : EVM.State} (h : EVMStateEquiv evm₁ evm₂)
    ...
    EVMStateEquiv
      (Storage.EVM.storageStore evm₁ evm₁.executionEnv.codeOwner slot val₁)
      (Storage.EVM.storageStore evm₂ evm₂.executionEnv.codeOwner slot val₂) :=
