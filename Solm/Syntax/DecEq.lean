...
`Expr`, `StorageType`, and `Stmt` each carry nested `List` payloads (e.g.
`arrayLit : List Expr`), a shape Lean's `deriving DecidableEq` handler cannot
process, so their instances are hand-written structural `decEq`s. Every other
type derives normally. Instances appear in dependency order: each is in scope
before the types that use it. Kept out of `Solm.Syntax.Basic` so that file reads
as plain type definitions.
-/
