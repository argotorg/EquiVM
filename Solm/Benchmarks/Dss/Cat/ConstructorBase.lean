import Solm.Benchmarks.Dss.Cat.Common
...
noncomputable def catCtorCode (vat : AccountAddress) : ByteArray :=
...
private theorem byteArray_write_from_ge_eq (src base : ByteArray) (srcAddr destAddr len : ℕ)
  ...
  rw [hcopy, htail]
  rw [show (ffi.ByteArray.zeroes (0 : ℕ)).data =
      (#[] : Array UInt8) from by
...
noncomputable def catCtorArgMem (vat : AccountAddress) : ByteArray :=
  (catCtorCode vat).write 3999 solcFreePtrMem 128 32

noncomputable def catCtorArgFreeMem (vat : AccountAddress) : ByteArray :=
  (UInt256.toByteArray ⟨160⟩).write 0 (catCtorArgMem vat) 64 32

theorem catCtorArgMem_eq (vat : AccountAddress) :
    catCtorArgMem vat =
      solcFreePtrMem ++ ffi.ByteArray.zeroes 32 ++ catCtorArgsTail vat := by
...
theorem catCtorArgMem_read128 (vat : AccountAddress) :
  ...
  rw [readWithPadding_eq_extract'
    (solcFreePtrMem ++ ffi.ByteArray.zeroes 32 ++ catCtorArgsTail vat)
    128 32 (by norm_num) (by norm_num) (by
      rw [ByteArray.size_append, hprefix, catCtorArgsTail_size])]
  rw [extract_append_right_window
    (solcFreePtrMem ++ ffi.ByteArray.zeroes 32) (catCtorArgsTail vat) 128
...
noncomputable def catCtorWardsHashMem (I : ExecutionEnv) (vat : AccountAddress) : ByteArray :=
...
theorem catCtorWardsHashSlot (I : ExecutionEnv) (vat : AccountAddress) :
    UInt256.ofNat (fromByteArrayBigEndian
        (ffi.KEC ((catCtorWardsHashMem I vat).readWithPadding 0 64))) =
...
noncomputable def catCtorReturnMem (I : ExecutionEnv) (vat : AccountAddress) : ByteArray :=
