import Solm.Benchmarks.Dss.Flipper.Common
...
/-! ## External-call transport helpers -/

-- LIBRARY CANDIDATE: expose the account-map extensional view used by external-call transport.
theorem flipper_accountMapExtensionalEq_of_accountMapEquiv {σ τ : AccountMap}
...
-- LIBRARY CANDIDATE: convert account-map extensional equality back to `accountMapEquiv`.
theorem flipper_accountMapEquiv_of_accountMapExtensionalEq {σ τ : AccountMap}
...
-- LIBRARY CANDIDATE: `typedCallViaEVM_accountMapEquiv` does not depend on equality of the
-- caller substates; this form returns the post-call `EVMStateEquiv` relation needed for call chains.
theorem flipper_typedCallViaEVM_accountMapEquiv_noSubstate
  ...
  | callMade hvalue hTheta hevm' hvalue' hdepth =>
      ...
      · have hσext : accountMapExtensionalEq σ' thetaRes.2.1 := hTheta_rel.2.2.2.2.2
        refine ⟨?_, rfl, ?_⟩
        · simp [hevm', hEnv]
        · simpa [hevm'] using flipper_accountMapEquiv_of_accountMapExtensionalEq hσext
