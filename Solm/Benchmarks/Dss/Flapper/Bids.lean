...
@[reducible] def flapperBidsStructGetterWf (code : ByteArray) (pc : UInt256) : Prop :=
  let p1 := pc + ⟨1⟩
...
theorem flapperBidsReturnBytes_size (bid lot guy tic endw : UInt256) :
  ...
  simp [flapperBidsReturnBytes, ByteArray.size_append]
...
theorem flapperBidsReturnMem_size {scratch : ByteArray}
    (bid lot guy tic endw : UInt256) (hscratch : scratch.size = 96) :
    (flapperBidsReturnMem scratch bid lot guy tic endw).size = 288 := by
  rw [flapperBidsReturnMem_eq bid lot guy tic endw hscratch]
  simp [ByteArray.size_append, ByteArray_zeroes_size, hscratch]

theorem flapperBidsReturnMem_read64 {scratch : ByteArray}
    (bid lot guy tic endw : UInt256)
    (hscratch : scratch.size = 96)
    (hread64 : scratch.readWithPadding 64 32 = UInt256.toByteArray ⟨128⟩) :
    (flapperBidsReturnMem scratch bid lot guy tic endw).readWithPadding 64 32 =
...
theorem flapperBidsReturnMem_mload64 {scratch : ByteArray}
    (bid lot guy tic endw : UInt256)
    (hscratch : scratch.size = 96)
    (hread64 : scratch.readWithPadding 64 32 = UInt256.toByteArray ⟨128⟩) :
    (if (⟨64⟩ : UInt256).toNat ≥ (flapperBidsReturnMem scratch bid lot guy tic endw).size
        ∨ (⟨64⟩ : UInt256) ≥ UInt256.ofNat 9 * ⟨32⟩ then ⟨0⟩
     else UInt256.ofNat
       (fromByteArrayBigEndian
        ((flapperBidsReturnMem scratch bid lot guy tic endw).readWithPadding
          (⟨64⟩ : UInt256).toNat 32)))
      = ⟨128⟩ :=
  mloadFreePtrValue
    (by rw [flapperBidsReturnMem_size bid lot guy tic endw hscratch]; decide)
    (by decide)
    (flapperBidsReturnMem_read64 bid lot guy tic endw hscratch hread64)

theorem flapperBidsReturnMem_read128_160 {scratch : ByteArray}
    (bid lot guy tic endw : UInt256) (hscratch : scratch.size = 96) :
    (flapperBidsReturnMem scratch bid lot guy tic endw).readWithPadding 128 160 =
      flapperBidsReturnBytes bid lot guy tic endw := by
  rw [flapperBidsReturnMem_eq bid lot guy tic endw hscratch]
...
theorem flapperBidsReturnEncoding (bid lot guy tic endw : UInt256) :
...
@[reducible] def flapperBidsReturnFromMemWf (code : ByteArray) (pc : UInt256) : Prop :=
  let p1 := pc + ⟨1⟩
  let p3 := p1 + UInt256.ofNat 2
  let p4 := p3 + ⟨1⟩
  let p5 := p4 + ⟨1⟩
...
theorem RD.flapperBidsReturnFromMem {code : ByteArray} {g : Sat256} {s0 : State}
    ...
    RDret code g s0 acc (flapperBidsReturnBytes bid lot guy tic endw) := by
  rcases hwf with
  ...
  have hmem5 : mem5 = flapperBidsReturnMem mem bid lot guy tic endw := by
    dsimp [mem5, mem4, mem3, mem2, mem1, endMasked, ticMasked, guyMasked,
      flapperBidsReturnMem, writeCascade, Reasoning.Theory.writeWord]
  have rd52 := rd51.mload 0 ⟨128⟩ (UInt256.ofNat 9) hd52 mem_cost
    (by
      rw [hmem5]
      exact flapperBidsReturnMem_mload64 bid lot guy tic endw hscratch hread64)
    (by decide) (by evm_ov)
  ...
  have hretBytes :
    ...
    rw [show (UInt256.add (UInt256.sub (⟨128⟩ : UInt256) ⟨128⟩) ⟨160⟩).toNat =
        160 from by decide]
    exact flapperBidsReturnMem_read128_160 bid lot guy tic endw hscratch
...
theorem flapperReachBidsBody {cA gh bl σ σ₀ A I} {g : Sat256}
  ...
  have htake :
      UInt256.eq
        (armSelNat flapperBytecode (nthArmPc flapperBytecode flapperLowLowFirstArmPc 4))
        (flapperSelWord I) ≠ ⟨0⟩ := by
    rw [hword]
    native_decide
  exact RD.dispatchTo ⟨433⟩ 4 hfirst
...
theorem flapperBidsBodyCoreOk
  ...
  have htoRoutine : ∃ k C, RD flapperBytecode I (Sat256.ofUInt256 g)
    ...
    have rd435 := rd432.push2 ⟨1562⟩ (by native_decide) (by evm_ov)
    exact ⟨_, _, by
  ...
  obtain ⟨_, _, hretPc⟩ := RD.flapperBidsStructGetter
    (code := flapperBytecode) (pc := ⟨1562⟩) (key := bidsArgWord I) (ret := ⟨462⟩)
    (R := [sel]) (by simpa using htoRoutineRd)
    (by
      unfold flapperBidsStructGetterWf
  ...
  have hval :
    ...
    simp [flapperAddressReturnWord, flapperUint48Offset20Word, flapperUint48Offset26Word,
      ...
      u256_land_comm]
  have henc :
      returnEquiv (flapperBidsReturnBytes bidWord lotWord packedWord ticRaw endRaw)
