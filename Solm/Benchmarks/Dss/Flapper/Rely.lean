import Solm.Benchmarks.Dss.Flapper.Dispatch
...
def relyAuthWord (σ : AccountMap) (I : ExecutionEnv) : UInt256 :=
  flapperSlotWord (relyAuthStorageSlot I) σ I
...
theorem evalExpr_rely_auth_true (evm : EVM.State) (I : ExecutionEnv)
  ...
  have hstorage :
    ...
    rw [evalExpr_storage_scalar_value
      ...
      (hloc := by rfl)
      (hload := by
        simpa [hload] using flapperStorageLocLoad_uint256 evm (relyAuthStorageSlot I))]
...
theorem evalExpr_rely_auth_false (evm : EVM.State) (I : ExecutionEnv)
  ...
  have hstorage :
    ...
    exact evalExpr_storage_scalar_value
      ...
      (hloc := by rfl)
      (hload := by
        exact flapperStorageLocLoad_uint256 evm (relyAuthStorageSlot I))
  have hne :
      Value.int (Int.ofNat
          (Solm.EVM.storageLoad evm evm.executionEnv.codeOwner
...
theorem relyNotAuthorizedWord :
    UInt256.shiftLeft
        (⟨0x119b185c1c195c8bdb9bdd0b585d5d1a1bdc9a5e9959⟩ : UInt256) ⟨82⟩ =
      ⟨0x466c61707065722f6e6f742d617574686f72697a656400000000000000000000⟩ := by
  native_decide

theorem flapperReachRelyBody {cA gh bl σ σ₀ A I} {g : Sat256}
    ...
    ∃ k C, RD flapperBytecode I g (initState cA gh bl σ σ₀ g A I)
        ⟨600⟩ [flapperSelWord I] solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty
        (cA, σ) k C := by
  have hword : flapperSelWord I = ⟨0x65fae35e⟩ :=
    by
  ...
  have heq0 : ∀ j, j < 2 →
      UInt256.eq (armSelNat flapperBytecode (nthArmPc flapperBytecode flapperLowHighFirstArmPc j))
        (flapperSelWord I) = ⟨0⟩ := by
    intro j hj
    interval_cases j <;> rw [hword] <;> native_decide
  have htake :
      UInt256.eq (armSelNat flapperBytecode (nthArmPc flapperBytecode flapperLowHighFirstArmPc 2))
        (flapperSelWord I) ≠ ⟨0⟩ := by
    rw [hword]
    native_decide
  exact RD.dispatchTo ⟨600⟩ 2 hfirst
    (fun j hj => flapperLowHighArmsWellFormed j (le_trans hj (by omega)))
    heq0 htake (by jump_dest) (by native_decide) (by simp)

theorem flapperRelyX_decoded {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
    (hsz36 : 36 ≤ I.calldata.size) (hsize : I.calldata.size < UInt256.size)
    (hreach : ∃ k C, RD flapperBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨600⟩ [sel]
      solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C) :
    ∃ k C, RD flapperBytecode I g (initState cA gh bl σ σ₀ g A I) ⟨2838⟩
        [relyUsrMaskedWord I, ⟨360⟩, sel]
        solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C := by
  obtain ⟨_, _, hdecoded⟩ := RD.solcOneAddressExternalLenOk
    (code := flapperBytecode) (sel := sel) (entry := ⟨600⟩) (ret := ⟨360⟩)
    (decoded := ⟨622⟩) hreach
  ...
  obtain ⟨_, _, hroutine⟩ := RD.solcOneAddressExternalMaskAndJumpMasked
    (code := flapperBytecode) (decoded := ⟨622⟩) (ret := ⟨360⟩) (routine := ⟨2838⟩)
    (R := [sel]) hdecoded
    (by native_decide) (by native_decide) (by native_decide) (by native_decide)
    (by native_decide) (by native_decide) (by native_decide) (by native_decide)
...
theorem flapperRelyX_shortarg {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
...
theorem flapperRelyX_authorized {cA σ I} {g : Sat256} {s0 : State} {k C : ℕ}
    {sel : UInt256} (hauth : relyAuthWord σ I = ⟨1⟩)
    (h : RD flapperBytecode I g s0 ⟨2838⟩
      [relyUsrMaskedWord I, ⟨360⟩, sel]
      solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C) :
    ∃ k' C', RD flapperBytecode I g s0 ⟨2931⟩
      [relyUsrMaskedWord I, ⟨360⟩, sel]
...
set_option maxHeartbeats 1000000 in
theorem flapperRelyX_storeAuthorized {cA σ I} {g : Sat256} {s0 : State} {k C : ℕ}
...
theorem flapperX_rely_unauthorized {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
    (hsz36 : 36 ≤ I.calldata.size) (hsize : I.calldata.size < UInt256.size)
    ...
    RDrev flapperBytecode g (initState cA gh bl σ σ₀ g A I) := by
  obtain ⟨_, _, rd1720⟩ := flapperRelyX_decoded (g := g) hsz36 hsize hreach
  exact flapperRelyX_unauthorized (I := I) hauth rd1720
...
theorem flapperRelyBodyCoreUnauthorized
    {cA gh bl σ_evm σ_solm σ₀ A I} {g : UInt256} {sel : UInt256}
    (hcode : I.code = flapperBytecode) (hsize : I.calldata.size < UInt256.size)
    (hwv : I.weiValue = ⟨0⟩)
    (hsz36 : 36 ≤ I.calldata.size)
  ...
  have hbody :
    ...
    simpa [evmSolm, relyAuthWord, flapperSlotWord, initState, Solm.EVM.storageLoad,
      State.lookupAccount] using
      flapperRelyBodyReverts evmSolm I
        (by simp only [evmSolm, initState]; exact hwv)
        (by simp [evmSolm, initState])
        hauthWord
  exact (flapperX_rely_unauthorized (g := Sat256.ofUInt256 g) hsz36 hsize hauth hreach)
    |>.reEquivExecutionRevert hcode hdispatch hdecode hbody

theorem flapperRelyBodyCoreDecodeFailed_short
    {cA gh bl σ_evm σ_solm σ₀ A I} {g : UInt256} {sel : UInt256}
    (hcode : I.code = flapperBytecode) (hsize : I.calldata.size < UInt256.size)
    (hsz4 : 4 ≤ I.calldata.size) (hshort : I.calldata.size < 36)
    (hdispatch : dispatchMsg contract I.calldata = some relyTransition)
...
theorem flapperRelyBodyCore {cA gh bl σ_evm σ_solm σ₀ A I} {g : UInt256}
  ...
  have hdispatch : dispatchMsg contract I.calldata = some relyTransition :=
    flapperDispatchRely hsel
  have hreach := flapperReachRelyBody (cA := cA) (gh := gh) (bl := bl)
    (σ := σ_evm) (σ₀ := σ₀) (A := A) (I := I) (g := Sat256.ofUInt256 g)
    hcode hwv hsz4 hsize hsel
  by_cases hsz36 : 36 ≤ I.calldata.size
  · by_cases hauth : relyAuthWord σ_evm I = ⟨1⟩
    · exact flapperRelyBodyCoreOk hcode hsize hperm hwv hsz36 hauth hdispatch
        (flapperDecode_rely_ok hsz36) hreach hAccounts
    · exact flapperRelyBodyCoreUnauthorized hcode hsize hwv hsz36 hauth hdispatch
        (flapperDecode_rely_ok hsz36) hreach hAccounts
  · exact flapperRelyBodyCoreDecodeFailed_short hcode hsize hsz4 (by omega)
      hdispatch hreach
