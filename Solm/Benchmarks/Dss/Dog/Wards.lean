import Solm.Benchmarks.Dss.Dog.Dispatch
    ...
    simpa using
      h125.selectorArmTaken (selNat := dogSelectorWord 16) (tgt := (⟨512⟩ : UInt256))
        ...
        (by
          ...
          native_decide)
        hwards
        (dogPatchedDJumpPrefix1405 ⟨512⟩ hpatch (by native_decide))
        (by simp)
  exact ⟨_, _, h512⟩
...
theorem dogWardsBodyCoreOk
  ...
  obtain ⟨_, _, hdecoded⟩ := RD.solcOneAddressExternalLenOk
    ...
    (by rw [dogDecodePatchedEqTemplate1405 hpatch (by native_decide)]; native_decide)
    (by rw [dogDecodePatchedEqTemplate1405 hpatch (by native_decide)]; native_decide)
    (dogPatchedDJumpPrefix1405 ⟨534⟩ hpatch (by native_decide)) hsz36 hsize
