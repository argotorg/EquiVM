import Solm.Benchmarks.Dss.Spot.Dispatch
...
theorem spotReachWardsBody {cA gh bl σ σ₀ A I} {g : Sat256}
...
theorem spotWardsBodyCoreOk
  ...
  have hbody :
      ExecTransitionBody config contract
        ...
        (.returned { contract := contract, locals := locals }
          ...
          (some [(.int (Int.ofNat (spotSlotWord (wardsMappingSlotFor I) σ_solm I).toNat))])) := by
  ...
  have hret :
    ...
    have hret' := RD.solcReturnWordFromMem
      (pc := ⟨336⟩) (val := spotSlotWord slot σ_evm I) (ret := ⟨336⟩) (R := [sel])
      (memout := solcScratchReturnMem (solcMappingHashMem ⟨0⟩ key)
        (spotSlotWord slot σ_evm I))
      ...
      (by rfl)
      (by
        exact solcScratchReturnMem_mload64 (spotSlotWord slot σ_evm I)
          (solcMappingHashMem_size ⟨0⟩ key) (solcMappingHashMem_read64 ⟨0⟩ key))
      (by
        exact solcScratchReturnMem_read128 (spotSlotWord slot σ_evm I)
  ...
  have hval :
      some [Value.int (Int.ofNat (spotSlotWord (wardsMappingSlotFor I) σ_solm I).toNat)] =
...
theorem spotWardsBodyCore {cA gh bl σ_evm σ_solm σ₀ A I} {g : UInt256}
  ...
  have hreach := spotReachWardsBody (cA := cA) (gh := gh) (bl := bl) (σ := σ_evm)
    (σ₀ := σ₀) (A := A) (I := I) (g := Sat256.ofUInt256 g)
    hcode hwv hsz4 hsize hsel
  by_cases hsz36 : 36 ≤ I.calldata.size
  · exact spotWardsBodyCoreOk hcode hwv hsz36 hsize hdispatch
      (spotDecode_wards_ok hsz36) hreach hAccounts
