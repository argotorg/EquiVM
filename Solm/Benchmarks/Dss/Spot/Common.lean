import Solm.Benchmarks.Dss.Spot.Bytecode
...
def spotSlotWord (slot : UInt256) (σ : AccountMap) (I : ExecutionEnv) : UInt256 :=
  solcSlotWord σ I slot

abbrev spotLiveWord (σ : AccountMap) (I : ExecutionEnv) : UInt256 :=
  spotSlotWord ⟨4⟩ σ I

abbrev spotAddressReturnWord (slot : UInt256) (σ : AccountMap) (I : ExecutionEnv) : UInt256 :=
  UInt256.land (spotSlotWord slot σ I) solcAddrMask

theorem spotStorageLocLoad_address_offset0 (evm : EVM.State) (slot : UInt256) :
...
theorem spotStorageLocLoad_uint256 (evm : EVM.State) (slot : UInt256) :
...
theorem spotAddressGetterBodyReturns (evm : EVM.State) (locals : Store)
  ...
  simpa [nonpayable] using
    nonpayableReturnExprBodyReturns (cfg := config) (contract := contract) h (by
      ...
      exact congrArg EvalResult.ok (spotStorageLocLoad_address_offset0 evm slot))
