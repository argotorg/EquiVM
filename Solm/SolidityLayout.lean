...
-- Sol⁻ names for the solc representation, now `Storage/SolcLayout.lean`.
export Storage (elemTypeSoliditySize checkBytesPacked bytesLikeLengthLoc solidityBytesDataBaseSlot solidityBytesDataSlot clearSolidityBytesDataWords clearSolidityBytesDataWordsFrom solidityDecodeBytesLengthHeader solidityBytesBaseSlotAndLength? solidityReadBytesLength? solidityBytesHeaderWord solidityBytesDataWordCount solidityShortBytesWord writeSolidityBytesDataWordsFrom readSolidityBytesDataWordsFrom solidityWord_toByteArray_size readSolidityBytesDataWordsFrom_size solidityReadBytesValue? solidityPrepareBytesWrite? solidityWriteBytesValue? solidityWriteValue? solidityReadValue? solidityClearValue? solidityStorageLayout)
open Storage
