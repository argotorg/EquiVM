...
mutual
  private def Stmt.decEq : (a b : Stmt) -> Decidable (a = b)
    ...
    | .internalCall nx ax rx, .internalCall ny ay ry =>
        ...
        | _, isFalse ha, _ => isFalse (by intro h; cases h; exact ha rfl)
        | _, _, isFalse hr => isFalse (by intro h; cases h; exact hr rfl)
    | .externalCall tx nx vx ax rx px, .externalCall ty ny vy ay ry py =>
        match Expr.decEq tx ty, (inferInstance : Decidable (nx = ny)), Expr.decEq vx vy,
            (inferInstance : Decidable (ax = ay)), (inferInstance : Decidable (rx = ry)),
            (inferInstance : Decidable (px = py)) with
        | isTrue ht, isTrue hn, isTrue hv, isTrue ha, isTrue hr, isTrue hp =>
            isTrue (by cases ht; cases hn; cases hv; cases ha; cases hr; cases hp; rfl)
        | isFalse ht, _, _, _, _, _ => isFalse (by intro h; cases h; exact ht rfl)
        | _, isFalse hn, _, _, _, _ => isFalse (by intro h; cases h; exact hn rfl)
        | _, _, isFalse hv, _, _, _ => isFalse (by intro h; cases h; exact hv rfl)
    ...
    | .checkedCall rx nx vx ax retx sx ex cx px,
        ...
        match Expr.decEq rx ry, (inferInstance : Decidable (nx = ny)), Expr.decEq vx vy,
            ...
            Stmt.decEqList sx sy, (inferInstance : Decidable (ex = ey)), Stmt.decEqList cx cy,
            (inferInstance : Decidable (px = py)) with
        | isTrue hr, isTrue hn, isTrue hv, isTrue ha, isTrue hret, isTrue hs, isTrue he,
            isTrue hc, isTrue hp =>
            isTrue (by
              cases hr; cases hn; cases hv; cases ha; cases hret; cases hs; cases he
              cases hc; cases hp; rfl)
        | isFalse hr, _, _, _, _, _, _, _, _ => isFalse (by intro h; cases h; exact hr rfl)
        | _, isFalse hn, _, _, _, _, _, _, _ => isFalse (by intro h; cases h; exact hn rfl)
        | _, _, isFalse hv, _, _, _, _, _, _ => isFalse (by intro h; cases h; exact hv rfl)
        | _, _, _, isFalse ha, _, _, _, _, _ => isFalse (by intro h; cases h; exact ha rfl)
        | _, _, _, _, isFalse hret, _, _, _, _ => isFalse (by intro h; cases h; exact hret rfl)
        | _, _, _, _, _, isFalse hs, _, _, _ => isFalse (by intro h; cases h; exact hs rfl)
        | _, _, _, _, _, _, isFalse he, _, _ => isFalse (by intro h; cases h; exact he rfl)
    ...
    | .externalCall _ _ _ _ _ _, .letDecl _ _ _ => isFalse (by intro h; cases h)
    ...
    | .externalCall _ _ _ _ _ _, .continue => isFalse (by intro h; cases h)
    ...
    | .letDecl _ _ _, .checkedCall _ _ _ _ _ _ _ _ _ => isFalse (by intro h; cases h)
    | .assign _ _ _, .checkedCall _ _ _ _ _ _ _ _ _ => isFalse (by intro h; cases h)
