...
theorem uadd3_ofNat_toNat {a b c : Nat}
  ...
  have habWord : (UInt256.ofNat a + UInt256.ofNat b).toNat = a + b := by
    simpa [UInt256.add] using uadd_ofNat_toNat ha hb hab
  rw [uadd_toNat, habWord, ulit_toNat' c hc,
    Nat.mod_eq_of_lt habc]
...
theorem slt_zero_low_high {a b : UInt256}
  ...
  have hbool : UInt256.sltBool a b = false := by
    ...
    rw [if_neg (show ¬ a.toNat ≥ 2 ^ 255 by omega),
      if_pos (show b.toNat ≥ 2 ^ 255 by omega)]
