  ...
  | `(evm_run $base:term with [ $steps,* ]) => do
      ...
      for s in steps.getElems do
        ...
        | `(evmStep| $op:ident $args*) =>
            ...
            -- with `decide +native` rather than `decide`.  `decode` kernel-reduces by scanning the
            -- bytecode `ByteArray` literal (O(pc) per step), so `decide` costs ~300–450ms per
            -- opcode on large bytecode; `decide +native` compiles the check and runs it in
            -- ~15ms.  This adds no new trust category: every `jump (by jump_dest)` already trusts the
            -- compiler via `decide +native`, so the proofs depend on it pervasively already.
            match op.getId with
            | `jump    => acc ← `($(acc).jump (by decide +native) $(args[0]!) (by evm_ov))
            | `jumpiT  => acc ← `($(acc).jumpiT (by decide +native) $(args[0]!) $(args[1]!) (by evm_ov))
            | `jumpiNT => acc ← `($(acc).jumpiNT (by decide +native) $(args[0]!) (by evm_ov))
            | _        => acc ← `($(acc).$op $args* (by decide +native) (by evm_ov))
