import Benchmarks.Auction.CreateAuctionFailureSelectorTrace
...
theorem auctionCreateAuction_mintCallFailureBubbleRevertTail {cA gh bl σInit σ₀ A I}
  ...
  have rd3166 := RD.returndatasize rd (by native_decide) (by omega)
...
theorem auctionCreateAuction_mintCallFailureSelectorToSwitch {cA gh bl σInit σ₀ A I}
  ...
  exact auctionMintFailureSelectorToSwitchDynamic rd (by native_decide) (by native_decide)
    hlen hosz hword hshr hov
...
theorem auctionCreateAuction_mintCallFailureSelectorSwitchDup {cA gh bl σInit σ₀ A I}
    ...
    (hov : R.length + 8 ≤ 1024) :
    ∃ k' C', RD auctionBytecode I g (initState cA gh bl σInit σ₀ g A I)
      auctionMintFailureAfterDupPc (sel :: sel :: R) memSel aw o acc k' C' := by
  have rd3124 := RD.dup1 rd (by native_decide) (by omega)
  exact ⟨_, _, by
...
theorem auctionCreateAuction_mintCallFailureSelectorSwitchPush {cA gh bl σInit σ₀ A I}
    ...
    {memSel o : ByteArray} {aw : UInt256} {k C : ℕ} {sel : UInt256} {R : List UInt256}
    (rd : RD auctionBytecode I g (initState cA gh bl σInit σ₀ g A I)
      auctionMintFailureAfterDupPc (sel :: sel :: R) memSel aw o acc k C)
    (hov : R.length + 8 ≤ 1024) :
    ∃ k' C', RD auctionBytecode I g (initState cA gh bl σInit σ₀ g A I)
      auctionMintFailureAfterPushSelectorPc
      (⟨0x08c379a0⟩ :: sel :: sel :: R) memSel aw o acc k' C' := by
  have rd3129 := RD.push4 rd ⟨0x08c379a0⟩ (by native_decide)
    (by simp only [List.length_cons]; omega)
  exact ⟨_, _, by
