    ...
    simpa using
      h147.selectorArmTaken (selNat := dogSelectorWord 9) (tgt := (⟨585⟩ : UInt256))
        ...
        (by
          ...
          decide +native)
        hfileAddress
        (dogPatchedDJumpPrefix1405 ⟨585⟩ hpatch (by decide +native))
        (by simp)
  exact ⟨_, _, h585⟩
...
theorem RD.dogFileAddressToSwitch {v : DogImmutables} {code : ByteArray}
  ...
  obtain ⟨_, _, hdecoded⟩ := RD.solcTwoAddressExternalLenOk
    ...
    (by rw [dogDecodePatchedEqTemplate1405 hpatch (by decide +native)]; decide +native)
    (by rw [dogDecodePatchedEqTemplate1405 hpatch (by decide +native)]; decide +native)
    (dogPatchedDJumpPrefix1405 ⟨607⟩ hpatch (by decide +native)) hsz68 hsize
  obtain ⟨_, _, hroutine⟩ := RD.dogFileAddressDecodeToRoutine
    (v := v) (code := code) (ret := ⟨313⟩) (sel := sel) (R := [])
    hpatch hdecoded (dogPatchedJumpDest hpatch (by decide +native))
    (by simp)
