...
@[simp] theorem bindParams_subFunction (x y : UInt256) :
    bindParams? subFunction.params [.int (Int.ofNat x.toNat), .int (Int.ofNat y.toNat)] =
      some (uintBinaryLocals x y) := by
  simpa [subFunction, uintBinaryLocals] using
    (bindParams_uint256_pair "x" "y" x y)

@[simp] theorem bindParams_minFunction (x y : UInt256) :
    bindParams? minFunction.params [.int (Int.ofNat x.toNat), .int (Int.ofNat y.toNat)] =
      some (uintBinaryLocals x y) := by
  simpa [minFunction, uintBinaryLocals] using
    (bindParams_uint256_pair "x" "y" x y)
  ...
  have hblock :
      ExecBlock config { contract := contract, locals := locals } evm
        ...
        (.returned { contract := contract, locals := localsZ } evm
          (some [.int (Int.ofNat x.toNat)])) := by
    refine ExecBlock.consNormal
      (ExecStmt.letDecl hmin (valueMatchesOptionalABIType_uint256_word x)) ?_
    exact ExecBlock.consReturn (ExecStmt.return (evalExprs?_singleton hz))
  ...
  have hblock :
      ExecBlock config { contract := contract, locals := locals } evm
        ...
        (.returned { contract := contract, locals := localsZ } evm
          (some [.int (Int.ofNat y.toNat)])) := by
    refine ExecBlock.consNormal
      (ExecStmt.letDecl hmin (valueMatchesOptionalABIType_uint256_word y)) ?_
    exact ExecBlock.consReturn (ExecStmt.return (evalExprs?_singleton hz))
  simpa [minFunction, locals, localsZ] using ExecFuncBody.execBlockRet hblock
