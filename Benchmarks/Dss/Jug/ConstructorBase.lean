...
theorem jugCtorArgMem_eq (vat : AccountAddress) :
    jugCtorArgMem vat =
      solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32) ++ jugCtorArgsTail vat := by
...
theorem jugCtorArgMem_read128 (vat : AccountAddress) :
  ...
  rw [readWithPadding_eq_extract'
    (solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32) ++ jugCtorArgsTail vat)
    128 32 (by norm_num) (by norm_num) (by
      rw [ByteArray.size_append, hprefix, jugCtorArgsTail_size])]
  rw [extract_append_right_window
    (solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32)) (jugCtorArgsTail vat) 128
