...
theorem jugDripSourceBodyVatFoldNoCodeReverts {cA gh bl σ σ₀ A I} {g : UInt256}
  ...
  have hbindRpow :
    ...
    simpa [jugUInt256Zero_toNat] using
      (bindParams_rpowFunction fee (⟨0⟩ : UInt256) jugRay)
  have hrpowBody :
      ∃ doneLocals,
  ...
  have hdiffReturn :
      ExecStmt config { contract := contract, locals := dripRateLocals I out fee jugRay rate }
        ...
        (.ok { contract := contract, locals := dripDeltaLocals I out fee jugRay rate ⟨0⟩ }
          evmVat) := by
    have h := internalCallFunctionReturn
      ...
      (execDiffFunctionReturn evmVat (x := rate) (y := rate) hprevMax hprevMax)
    simpa [resumeAfterInternalCall, dripDeltaLocals, hdiffZero] using h
  have hvatFold :
      evalExpr? config
          { contract := contract, locals := dripDeltaLocals I out fee jugRay rate ⟨0⟩ } evmVat
          (.storage vatRef) =
        .ok (.address (dripVatAddress evmVat.accountMap evmVat.executionEnv)) := by
    exact evalExpr_dripStorageVatOfLocals
      (evm := evmVat) (locals := dripDeltaLocals I out fee jugRay rate ⟨0⟩)
      (dripDeltaLocals_get_vat I out fee jugRay rate ⟨0⟩)
  have hfoldGuard :
      evalExpr? config
          { contract := contract, locals := dripDeltaLocals I out fee jugRay rate ⟨0⟩ } evmVat
          (.binary .gt (.extCodeSize (.storage vatRef)) (.intLit 0)) =
  ...
  have hblock :
    ...
    exact ExecBlock.consRevert (by
      simpa [locals, dripVatIlksLocals, dripVatIlksPrevLocals, dripFeeLocals, dripPowLocals,
        dripRateLocals, dripDeltaLocals, collapseReturns] using
      ...
      evalExprs? config
        ...
        .ok [.fixedBytes bytes32Width (fileDutyIlkBytes I),
          .address (AccountAddress.ofUInt256
            (dripVowTargetWord evmVat.accountMap evmVat.executionEnv)),
          .int (Int.ofNat (⟨0⟩ : UInt256).toNat)] :=
    evalExprs_dripVatFoldArgs evmVat I out fee jugRay rate ⟨0⟩
  have hprevStmt :
      ExecStmt config { contract := contract, locals := dripVatIlksLocals I out } evmVat
        (.letDecl "prev" (some uint256) (.tupleGet (.var "vatIlk") 1))
        (.ok { contract := contract, locals := dripVatIlksPrevLocals I out } evmVat) := by
...
theorem jugDripSourceBodyVatFoldCallSucceededReturns {cA gh bl σ σ₀ A I} {g : UInt256}
  ...
  have hfoldReturn :
    ...
    have h := ExecStmt.externalCallSuccess (cfg := config)
      (solm := { contract := contract, locals := dripDeltaLocals I out fee jugRay rate ⟨0⟩ })
      ...
      hvatFold (by simp [evalExpr?, pure]) hfoldArgs hfoldCall hfoldDecode
    simpa [foldLocals, collapseReturns] using h
  have hfoldLocalsIlks : foldLocals.get? "ilks" = none := by
    dsimp [foldLocals]
    change ((dripDeltaLocals I out fee jugRay rate ⟨0⟩).insert "_foldRet" .unit).get?
        "ilks" = none
    rw [store_get_ne (dripDeltaLocals I out fee jugRay rate ⟨0⟩)
      (k := "_foldRet") (a := "ilks") .unit (by decide)]
    exact dripDeltaLocals_get_ilks I out fee jugRay rate ⟨0⟩
  have hfoldLocalsIlk :
    ...
    exact dripDeltaLocals_get_ilk I out fee jugRay rate ⟨0⟩
  have hfoldLocalsRate :
    ...
    exact dripDeltaLocals_get_rate I out fee jugRay rate ⟨0⟩
  have htimestamp :
