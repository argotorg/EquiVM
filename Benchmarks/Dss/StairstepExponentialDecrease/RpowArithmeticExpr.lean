...
theorem rpowRay_mul_div_cancel (y : UInt256)
    (hfit : rpowRay.toNat * y.toNat < UInt256.size) :
    UInt256.div (y * rpowRay) rpowRay = y := by
  apply u256_inj
  rw [udiv_toNat]
  have hfit' : y.toNat * rpowRay.toNat < UInt256.size := by
    simpa [Nat.mul_comm] using hfit
  have hprod : (y * rpowRay).toNat = y.toNat * rpowRay.toNat := by
    rw [umul_toNat y rpowRay hfit']
  have hRayPos : 0 < rpowRay.toNat := by
    rw [rpowRay_toNat]
    norm_num
  rw [hprod]
...
theorem evalExpr_mod_int_ok {evm : EVM.State} {locals : Store}
    ...
    (hrhs : evalExpr? config { contract := contract, locals := locals } evm rhs =
      .ok (.int (Int.ofNat b.toNat)))
    (hb : b ≠ ⟨0⟩) (hr : r = Int.ofNat a.toNat % Int.ofNat b.toNat) :
    evalExpr? config { contract := contract, locals := locals } evm (.binary (.mod (.uint ⟨256, by decide⟩)) lhs rhs) =
      .ok (.int r) := by
  subst r
  simpa only [Int.natCast_emod] using evalExpr_mod_uint256_words_ok hlhs hrhs hb
