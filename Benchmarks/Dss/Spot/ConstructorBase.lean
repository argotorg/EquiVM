...
theorem spotCtorArgMem_eq (vat : AccountAddress) :
    spotCtorArgMem vat =
      solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32) ++ spotCtorArgsTail vat := by
...
theorem spotCtorArgMem_read128 (vat : AccountAddress) :
  ...
  rw [readWithPadding_eq_extract'
    (solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32) ++ spotCtorArgsTail vat)
    128 32 (by norm_num) (by norm_num) (by
      rw [ByteArray.size_append, hprefix, spotCtorArgsTail_size])]
  rw [extract_append_right_window
    (solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32)) (spotCtorArgsTail vat) 128
