...
theorem potCtorArgMem_eq (vat : AccountAddress) :
    potCtorArgMem vat =
      solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32) ++ potCtorArgsTail vat := by
...
theorem potCtorArgMem_read128 (vat : AccountAddress) :
  ...
  rw [readWithPadding_eq_extract'
    (solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32) ++ potCtorArgsTail vat)
    128 32 (by norm_num) (by norm_num) (by
      rw [ByteArray.size_append, hprefix, potCtorArgsTail_size])]
  rw [extract_append_right_window
    (solcFreePtrMem ++ ffi.ByteArray.zeroes (USize.ofNat 32)) (potCtorArgsTail vat) 128
