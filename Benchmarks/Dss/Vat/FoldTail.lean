...
theorem vatFoldSourceRevertAfterRateBlock (evm : EVM.State) (I : ExecutionEnv)
  ...
  have h01 := Reasoning.Refinement.execBlock_append hprefix hrateRevert
...
theorem vatFoldSourceRevertAfterRadBlock (evm evmRate : EVM.State) (I : ExecutionEnv)
  ...
  have hprefix :
    ...
    refine ExecBlock.consNormal (ExecStmt.requireTrue hauth) ?_
    exact ExecBlock.consNormal (ExecStmt.requireTrue hlive) ExecBlock.nil
  have h01 := Reasoning.Refinement.execBlock_append hprefix hrateOk
  have h02 := Reasoning.Refinement.execBlock_append h01 hradRevert
  have hblock := Reasoning.Refinement.execBlock_append_term
    (s2 :=
      checkedAddSignedInto "daiNew" (.storage (daiRef (.var "u"))) (.var "rad") ++
      [ .assign .storage (daiRef (.var "u")) (.var "daiNew") ] ++
...
theorem vatFoldSourceSuccess (evm : EVM.State) (I : ExecutionEnv)
  ...
  have h01 := Reasoning.Refinement.execBlock_append hprefix hrateBlock
  have h02 := Reasoning.Refinement.execBlock_append h01 hradBlock
  have h03 := Reasoning.Refinement.execBlock_append h02 hdaiBlock
  have h04 := Reasoning.Refinement.execBlock_append h03 hdaiAssign
  have h05 := Reasoning.Refinement.execBlock_append h04 hdebtBlock
  have hblock := Reasoning.Refinement.execBlock_append h05 hdebtAssign
  simpa [ExecTransitionBody, foldTransition, nonpayable, auth, requireLive,
