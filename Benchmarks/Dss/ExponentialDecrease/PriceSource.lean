...
theorem priceLocalsN_get_top (σ : AccountMap) (I : ExecutionEnv) :
  ...
  rw [priceLocalsN, priceLocals_get_top]

theorem priceLocalsN_get_dur (σ : AccountMap) (I : ExecutionEnv) :
    (priceLocalsN σ I).get? "dur" = some (.int (Int.ofNat (priceN σ I).toNat)) := by
  rw [priceLocalsN, priceN, priceLocals_get_dur]

theorem priceLocalsN_get_cut_none (σ : AccountMap) (I : ExecutionEnv) :
    (priceLocalsN σ I).get? "cut" = none := by
  rw [priceLocalsN, priceLocals]
  rw [store_get_ne, store_get_ne]
  · simp
  all_goals native_decide
...
theorem evalExpr_priceCut_word {evm : EVM.State} {locals : Store} {cut : UInt256}
    ...
    (hcut : Solm.EVM.storageLoad evm evm.executionEnv.codeOwner ⟨1⟩ = cut) :
...
theorem evalExprs_priceRpowArgsNZero {evm : EVM.State} {σ : AccountMap} {I : ExecutionEnv}
    (hcut : Solm.EVM.storageLoad evm evm.executionEnv.codeOwner ⟨1⟩ = priceCutWord σ I)
    (hn : priceN σ I = ⟨0⟩) :
    evalExprs? config { contract := contract, locals := priceLocalsN σ I } evm
      [.storage cutRef, .var "dur", .intLit RAY] =
  ...
  have hnEval :
      evalExpr? config { contract := contract, locals := priceLocalsN σ I } evm
        (.var "dur") = .ok (.int (Int.ofNat (priceN σ I).toNat)) :=
    evalExpr_price_varUInt256 (priceLocalsN_get_dur σ I)
  have hRay :
      evalExpr? config { contract := contract, locals := priceLocalsN σ I } evm
...
theorem stairstepPriceSourceXZeroNNonzeroReturns {evm : EVM.State} {σ : AccountMap}
    ...
    (hcutLoad :
      Solm.EVM.storageLoad evm evm.executionEnv.codeOwner ⟨1⟩ = priceCutWord σ I)
    (hn : priceN σ I ≠ ⟨0⟩)
  ...
  have hbindRpow :
    ...
    simpa [rpowFunction, uintTernaryLocals, hcut] using
      bindParams_uint256_triple "x" "n" "b" (⟨0⟩ : UInt256) (priceN σ I) stairstepRay
  have hrpowReturn :
    ...
    have h := internalCallFunctionReturn
      ...
      (args := [.storage cutRef, .var "dur", .intLit RAY])
  ...
  have hbindRmul :
    ...
    simpa [rmulFunction, priceUintBinaryLocals] using
      bindParams_uint256_pair "x" "y" (priceTop I) (⟨0⟩ : UInt256)
...
theorem stairstepPriceSourceNZeroReturns {evm : EVM.State} {σ : AccountMap}
  ...
  have hbindRpow :
    ...
    simpa [rpowFunction, priceUintTernaryLocals] using
      bindParams_uint256_triple "x" "n" "b" (priceCutWord σ I) (⟨0⟩ : UInt256)
        stairstepRay
  have hrpowReturn :
      ExecStmt config { contract := contract, locals := priceLocalsN σ I } evm
        (.internalCall "rpow" [.storage cutRef, .var "dur", .intLit RAY] "pow")
  ...
  have hbindRmul :
    ...
    simpa [rmulFunction, priceUintBinaryLocals] using
      bindParams_uint256_pair "x" "y" (priceTop I) stairstepRay
...
theorem stairstepPriceSourceRpowReverts {evm : EVM.State} {σ : AccountMap}
  ...
  have hbindRpow :
    ...
    simpa [rpowFunction, uintTernaryLocals] using
      bindParams_uint256_triple "x" "n" "b" (priceCutWord σ I) (priceN σ I)
        stairstepRay
  have hrpowRevert :
      ExecStmt config { contract := contract, locals := priceLocalsN σ I } evm
        (.internalCall "rpow" [.storage cutRef, .var "dur", .intLit RAY] "pow")
...
theorem stairstepPriceSourceRpowReturns {evm : EVM.State} {σ : AccountMap}
  ...
  have hbindRmul :
    ...
    simpa [rmulFunction, priceUintBinaryLocals] using
      bindParams_uint256_pair "x" "y" (priceTop I) pow
