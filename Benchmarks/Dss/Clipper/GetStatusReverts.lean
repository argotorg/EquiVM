  ...
  have hbody :
      ExecBlock (config v) startFrame evm
        ([ .internalCall "sub" [.env .timestamp, .var "tic"] "ageForPrice" ] ++
          callStmts ++ tailStmts)
        .reverted :=
    Reasoning.Refinement.execBlock_append (s2 := callStmts ++ tailStmts) hsubPrice htail
  apply ExecFuncBody.execBlockRevert
...
theorem clipperStatusFunctionRevertsAgeForDone (v : ClipperImmutables)
  ...
  have hcallTail :
    ...
    Reasoning.Refinement.execBlock_append (s2 := tailStmts) hcallBlock htail
  have hbody :
      ExecBlock (config v) startFrame evm
        ([ .internalCall "sub" [.env .timestamp, .var "tic"] "ageForPrice" ] ++
          callStmts ++ tailStmts)
        .reverted :=
    Reasoning.Refinement.execBlock_append (s2 := callStmts ++ tailStmts) hsubPrice hcallTail
  apply ExecFuncBody.execBlockRevert
  simpa [statusFunction, checkedExternalCallStmts, startFrame, callStmts, tailStmts] using hbody
...
private theorem clipperStatusFunctionRevertsRdivCore (v : ClipperImmutables)
  ...
  have hcallTail :
      ExecBlock (config v) ageFrame evm (callStmts ++ tailStmts) .reverted :=
    Reasoning.Refinement.execBlock_append (s2 := tailStmts) hcallBlock htailBlock
  have hbody :
      ExecBlock (config v) startFrame evm
        ([ .internalCall "sub" [.env .timestamp, .var "tic"] "ageForPrice" ] ++
          callStmts ++ tailStmts)
