...
theorem clipperYankX_dogDigsNoCode {cA σ I} {g : Sat256} {s0 : State}
    ...
    (hcodeSizeDog : Reasoning.Theory.uniswapExtCodeSizeWord σ target = ⟨0⟩) :
    RDrev code g s0 := by
  exact RD.uniswapExtcodesizeGuardMissing (pc := ⟨2301⟩) (okPc := ⟨2313⟩)
    rd2301 hcodeSizeDog
...
theorem clipperYankX_dogDigsPostCall {cA gh bl σ₀ σStart σ I}
  ...
  obtain ⟨_, _, _, rd2316⟩ :=
    RD.uniswapExtcodesizeGuardOkGas (pc := ⟨2301⟩) (okPc := ⟨2313⟩)
      rd2301 hcodeSizeDog
...
theorem RD.clipperYankDogDigsCallFailure
    ...
    RDrev code g s0 := by
  exact RD.uniswapCallSuccessGuardMissing (pc := ⟨2317⟩) (okPc := ⟨2333⟩)
...
theorem RD.clipperYankDogDigsCallSuccessToFluxSetup
    ...
    ∃ k' C', RD code ee g s0 ⟨2335⟩
      (⟨196⟩ :: clipperDogDigsSelectorWord :: target ::
        clipperYankArgWord ee :: ⟨502⟩ :: [sel])
      mem aw o acc k' C' := by
  exact RD.uniswapCallSuccessGuardOk (pc := ⟨2317⟩) (okPc := ⟨2333⟩) rd
