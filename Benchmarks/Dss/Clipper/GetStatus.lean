...
theorem clipperGetStatusBody (v : ClipperImmutables) {code : ByteArray}
  ...
  · have hdec := clipperDecode_getStatus_ok v (I := I) hsz36
    ...
    have _hstatusPricePostCall :
        ...
        (hcalcCode :
          Reasoning.Theory.uniswapExtCodeSizeWord σ_evm calcAddr ≠ ⟨0⟩)
      ...
      · have hrev :=
        ...
        have hcalcCodeSolmNE :
            Reasoning.Theory.uniswapExtCodeSizeWord σ_solm calcAddr ≠ ⟨0⟩ := by
          intro hzero
          exact hcalcCode (by
            rw [Reasoning.Theory.uniswapExtCodeSizeWord_accountMapEquiv hAccounts calcAddr]
            exact hzero)
        have hcalcCodeSolm :
            0 < (UInt256.ofNat
