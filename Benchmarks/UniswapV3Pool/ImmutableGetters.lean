...
theorem uniswapV3PoolFactoryPatchWord8315 {v : PoolImmutables} {code : ByteArray}
    (hpatch : patchRuntime uniswapV3PoolBytecode (patches v) = some code) :
    code.extract 8315 8347 = UInt256.toByteArray (EVM.Word.ofNat v.factory.toNat) := by
  let value := UInt256.toByteArray (EVM.Word.ofNat v.factory.toNat)
  let post : List (Nat × ByteArray) :=
    [(8829, value), (10457, value),
  ...
  simpa using
    (patchRuntime_extract_patch (template := uniswapV3PoolBytecode) (out := code)
      (value := value) (pre := []) (post := post) (offset := 8315) hsize hpost hpatch')

theorem uniswapV3PoolFactoryConstDecode8314 {v : PoolImmutables} {code : ByteArray}
    (hpatch : patchRuntime uniswapV3PoolBytecode (patches v) = some code) :
    decode code ⟨8314⟩ =
      some (.Push .PUSH32, some (EVM.Word.ofNat v.factory.toNat, 32)) := by
  have hsize := uniswapV3PoolPatchedSize hpatch
  have hget : code.get? ({ val := 8314 } : UInt256).toNat =
      uniswapV3PoolBytecode.get? ({ val := 8314 } : UInt256).toNat := by
    change code.get? 8314 = uniswapV3PoolBytecode.get? 8314
    apply get?_eq_of_extract_one
    · rw [hsize]
      native_decide
    · native_decide
    · exact patchRuntime_extract_eq (start := 8314) (stop := 8315)
  ...
  have hextract : code.extract' ({ val := 8314 } : UInt256).toNat.succ
      (({ val := 8314 } : UInt256).toNat.succ + 32) =
      UInt256.toByteArray (EVM.Word.ofNat v.factory.toNat) := by
    change code.extract' 8315 8347 =
      UInt256.toByteArray (EVM.Word.ofNat v.factory.toNat)
    unfold ByteArray.extract'
    have hguard : (decide (8315 < 2 ^ 64) && decide (8347 < 2 ^ 64)) = true := by
      native_decide
    rw [if_pos hguard]
    exact uniswapV3PoolFactoryPatchWord8315 hpatch
  have hgetSome : code.get? ({ val := 8314 } : UInt256).toNat = some 0x7f := by
    rw [hget]
    native_decide
  ...
  change some (Operation.Push Operation.POp.PUSH32,
      some (uInt256OfByteArray
        (code.extract' ({ val := 8314 } : UInt256).toNat.succ
          (({ val := 8314 } : UInt256).toNat.succ + 32)), 32)) =
