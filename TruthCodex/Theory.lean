...
@[simp] lemma default_eq_empty :
    (default : ByteArray) = ByteArray.empty := by
  rfl
...
lemma toList_loop_eq_reverse_append_drop_data (bs : ByteArray) (i : Nat)
    (r : List UInt8) :
    ByteArray.toList.loop bs i r = r.reverse ++ bs.data.toList.drop i := by
  rw [ByteArray.toList.loop.eq_def]
  by_cases h : i < bs.size
  · rw [if_pos h]
    rw [toList_loop_eq_reverse_append_drop_data bs (i + 1) (bs.get! i :: r)]
    have hLen : i < bs.data.toList.length := by
      simpa [Array.length_toList, ByteArray.size_data] using h
    rw [List.drop_eq_getElem_cons hLen]
    have hGet : bs.get! i = bs.data.toList[i] := by
      rw [Array.getElem_toList]
      exact getElem!_pos bs.data i (by simpa [ByteArray.size_data] using h)
    rw [hGet]
...
lemma toList_eq_data_toList (bs : ByteArray) :
    bs.toList = bs.data.toList := by
  rw [ByteArray.toList.eq_1]
  simpa using toList_loop_eq_reverse_append_drop_data bs 0 []

lemma toList_append (a b : ByteArray) :
    (a ++ b).toList = a.toList ++ b.toList := by
  rw [toList_eq_data_toList, toList_eq_data_toList, toList_eq_data_toList]
...
lemma write_size_ge_before_of_stop_le
    {source dest : ByteArray} {sourceAddr destAddr len start stop : Nat}
    (hStartStop : start < stop)
    (hStopDest : stop ≤ dest.size)
    (hStopDestAddr : stop ≤ destAddr) :
    stop ≤ (source.write sourceAddr dest destAddr len).size := by
  let written := source.write sourceAddr dest destAddr len
  change stop ≤ written.size
  have hExtract :
      written.extract start stop = dest.extract start stop := by
    dsimp [written]
    exact write_extract_before_of_stop_le hStopDest hStopDestAddr
  have hSize := congrArg ByteArray.size hExtract
  rw [ByteArray.size_extract, ByteArray.size_extract] at hSize
  by_contra hStopWritten
  have hWrittenLt : written.size < stop := by omega
  have hMinWritten : min stop written.size = written.size :=
    Nat.min_eq_right (by omega)
  have hMinDest : min stop dest.size = stop :=
    Nat.min_eq_left hStopDest
  rw [hMinWritten, hMinDest] at hSize
  omega
...
namespace Ethereum.MachineState

lemma M_32_mul_32_le_add_63_of_active_mul_le
    {activeWords start : Nat}
    (hActive : activeWords * 32 ≤ start + 63) :
    M activeWords start 32 * 32 ≤ start + 63 := by
  simp [M]
  by_cases hMax : activeWords ≤ (start + 32 + 31) / 32
  · rw [max_eq_right hMax]
    have hDiv := Nat.div_mul_le_self (start + 32 + 31) 32
    omega
  · have hDivLe : (start + 32 + 31) / 32 ≤ activeWords :=
      le_of_not_ge hMax
    rw [max_eq_left hDivLe]
    exact hActive

end Ethereum.MachineState
...
lemma toNat_mul_of_lt {x y : Ethereum.UInt256}
    (h : x.toNat * y.toNat < Ethereum.UInt256.size) :
    (x * y).toNat = x.toNat * y.toNat := by
  cases x with
  | mk xv =>
      ...
      | mk yv =>
          unfold Ethereum.UInt256.toNat at h ⊢
          unfold HMul.hMul instHMul Mul.mul instMul UInt256.mul
...
lemma activeWords_mul_32_gt_64_of_M_3_32
    {ptr : Ethereum.UInt256}
    (hPtrGe : 128 ≤ ptr.toNat)
    (hPtrAdd63 : ptr.toNat + 63 < Ethereum.UInt256.size) :
    64 <
      ((Ethereum.UInt256.ofNat (Ethereum.MachineState.M 3 ptr.toNat 32)) *
        (⟨32⟩ : Ethereum.UInt256)).toNat := by
  let m := Ethereum.MachineState.M 3 ptr.toNat 32
  have hActiveLe : 3 * 32 ≤ ptr.toNat + 63 := by
    omega
  have hMLe : m * 32 ≤ ptr.toNat + 63 := by
    dsimp [m]
    exact Ethereum.MachineState.M_32_mul_32_le_add_63_of_active_mul_le hActiveLe
  have hMGe : 3 ≤ m := by
    dsimp [m]
    simp [Ethereum.MachineState.M]
  have hMLt : m * 32 < Ethereum.UInt256.size := by
    omega
  have hMlt : m < Ethereum.UInt256.size := by
    omega
  have hMNat :
      (Ethereum.UInt256.ofNat m).toNat = m :=
    Ethereum.UInt256.ofNat_toNat_of_lt hMlt
  have h32 :
      (⟨32⟩ : Ethereum.UInt256).toNat = 32 := by
    rfl
  have hMulLt :
      (Ethereum.UInt256.ofNat m).toNat *
        (⟨32⟩ : Ethereum.UInt256).toNat < Ethereum.UInt256.size := by
    rw [hMNat, h32]
    exact hMLt
  rw [Ethereum.UInt256.toNat_mul_of_lt hMulLt, hMNat, h32]
  omega

lemma three_mul_32_gt_64 :
    64 <
      ((⟨3⟩ : Ethereum.UInt256) *
        (⟨32⟩ : Ethereum.UInt256)).toNat := by
  have h3 : (⟨3⟩ : Ethereum.UInt256).toNat = 3 := by
    rfl
  have h32 : (⟨32⟩ : Ethereum.UInt256).toNat = 32 := by
    rfl
  have hMulLt :
      (⟨3⟩ : Ethereum.UInt256).toNat *
        (⟨32⟩ : Ethereum.UInt256).toNat < Ethereum.UInt256.size := by
    rw [h3, h32]
    norm_num [Ethereum.UInt256.size]
  rw [Ethereum.UInt256.toNat_mul_of_lt hMulLt, h3, h32]
  norm_num
...
def subCosts (x : Ethereum.UInt256) : List Nat → Ethereum.UInt256
  | [] => x
  | cost :: costs => subCosts (x - Ethereum.UInt256.ofNat cost) costs

lemma toNat_subCosts_of_sum_le
    {x : Ethereum.UInt256} {costs : List Nat}
    (hCostSize : ∀ cost ∈ costs, cost < Ethereum.UInt256.size)
    (hSum : costs.sum ≤ x.toNat) :
    (subCosts x costs).toNat = x.toNat - costs.sum := by
  induction costs generalizing x with
  | nil =>
      simp [subCosts]
  | cons cost costs ih =>
      have hSumCons : cost + costs.sum ≤ x.toNat := by
        simpa using hSum
      have hCost : cost < Ethereum.UInt256.size := hCostSize cost (by simp)
      have hCosts :
          ∀ tailCost ∈ costs, tailCost < Ethereum.UInt256.size := by
        intro tailCost hTail
        exact hCostSize tailCost (by simp [hTail])
      have hHead : cost ≤ x.toNat := by
        omega
      have hSub :
          (x - Ethereum.UInt256.ofNat cost).toNat = x.toNat - cost :=
        toNat_sub_ofNat_of_le hCost hHead
      have hTailSum : costs.sum ≤ (x - Ethereum.UInt256.ofNat cost).toNat := by
        rw [hSub]
        omega
      rw [subCosts, ih hCosts hTailSum, hSub]
      have hNat : x.toNat - cost - costs.sum =
          x.toNat - (cost + costs.sum) := by
        omega
      simpa using hNat

lemma sum_le_toNat_of_subCosts_not_lt
    {x : Ethereum.UInt256} {costs : List Nat} {remaining : Nat}
    (hCostSize : ∀ cost ∈ costs, cost < Ethereum.UInt256.size)
    (hSum : costs.sum ≤ x.toNat)
    (hRemaining : ¬ (subCosts x costs).toNat < remaining) :
    costs.sum + remaining ≤ x.toNat := by
  have hSub := toNat_subCosts_of_sum_le
    (x := x) (costs := costs) hCostSize hSum
...
lemma fromBytesBigEndian_append_128_add_63_lt_uint256_size
    {xs : List UInt8} (hLen : xs.length ≤ 31) :
    fromBytesBigEndian (xs ++ [0x80]) + 63 < UInt256.size := by
  rw [fromBytesBigEndian_append_singleton]
  change 128 + 256 * fromBytesBigEndian xs + 63 < UInt256.size
  have hPrefix : fromBytesBigEndian xs < 2 ^ (8 * 31) :=
    fromBytesBigEndian_lt_of_length_le hLen
  have hPrefixLe : fromBytesBigEndian xs ≤ 2 ^ (8 * 31) - 1 :=
    Nat.le_sub_one_of_lt hPrefix
...
lemma fromByteArrayBigEndian_append_singleton (pre : ByteArray) (b : UInt8) :
    fromByteArrayBigEndian (pre ++ ByteArray.mk #[b]) =
      b.toNat + 256 * fromByteArrayBigEndian pre := by
  unfold fromByteArrayBigEndian
  rw [ByteArray.toList_append]
  simp [ByteArray.toList_eq_data_toList, fromBytesBigEndian_append_singleton]

lemma fromByteArrayBigEndian_append_128_add_32_lt_uint256_size
    {pre : ByteArray} (hLen : pre.size ≤ 31) :
    fromByteArrayBigEndian (pre ++ ByteArray.mk #[0x80]) + 32 < UInt256.size := by
  unfold fromByteArrayBigEndian
  rw [ByteArray.toList_append]
  simp [ByteArray.toList_eq_data_toList]
  apply fromBytesBigEndian_append_128_add_32_lt_uint256_size
  rw [Array.length_toList, ByteArray.size_data]
  exact hLen

lemma fromByteArrayBigEndian_append_128_add_63_lt_uint256_size
    {pre : ByteArray} (hLen : pre.size ≤ 31) :
    fromByteArrayBigEndian (pre ++ ByteArray.mk #[0x80]) + 63 < UInt256.size := by
  unfold fromByteArrayBigEndian
  rw [ByteArray.toList_append]
  simp [ByteArray.toList_eq_data_toList]
  apply fromBytesBigEndian_append_128_add_63_lt_uint256_size
  rw [Array.length_toList, ByteArray.size_data]
  exact hLen
...
lemma fromByteArrayBigEndian_toByteArray_128_add_32_lt_size :
    Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray) + 32 <
      Ethereum.UInt256.size := by
  rw [toByteArray_128_eq_zeroes_append_128]
  apply Ethereum.fromByteArrayBigEndian_append_128_add_32_lt_uint256_size
  rw [ByteArray_zeroes_size]
  change (31#System.Platform.numBits).toNat ≤ 31
  rw [BitVec.toNat_ofNat]
  apply Nat.mod_le

lemma fromByteArrayBigEndian_toByteArray_128_add_63_lt_size :
    Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray) + 63 <
      Ethereum.UInt256.size := by
  rw [toByteArray_128_eq_zeroes_append_128]
  apply Ethereum.fromByteArrayBigEndian_append_128_add_63_lt_uint256_size
  rw [ByteArray_zeroes_size]
  change (31#System.Platform.numBits).toNat ≤ 31
  rw [BitVec.toNat_ofNat]
  apply Nat.mod_le

lemma ofNat_fromByteArrayBigEndian_toByteArray_128_add_32_lt_size :
    (Ethereum.UInt256.ofNat
        (Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray))).toNat +
      32 < Ethereum.UInt256.size := by
  have hSmall := fromByteArrayBigEndian_toByteArray_128_add_32_lt_size
  have hVal :
      (Ethereum.UInt256.ofNat
          (Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray))).toNat =
        Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray) :=
    Ethereum.UInt256.ofNat_toNat_of_lt (by omega)
  rw [hVal]
  exact hSmall

lemma ofNat_fromByteArrayBigEndian_toByteArray_128_add_63_lt_size :
    (Ethereum.UInt256.ofNat
        (Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray))).toNat +
      63 < Ethereum.UInt256.size := by
  have hSmall := fromByteArrayBigEndian_toByteArray_128_add_63_lt_size
  have hVal :
      (Ethereum.UInt256.ofNat
          (Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray))).toNat =
        Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray) := by
    exact Ethereum.UInt256.ofNat_toNat_of_lt (by omega)
  rw [hVal]
  exact hSmall

lemma ofNat_fromByteArrayBigEndian_toByteArray_128_ge_128 :
    128 ≤
      (Ethereum.UInt256.ofNat
        (Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray))).toNat := by
  have hNaturalSmall := fromByteArrayBigEndian_toByteArray_128_add_32_lt_size
  have hVal :
      (Ethereum.UInt256.ofNat
          (Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray))).toNat =
        Ethereum.fromByteArrayBigEndian ((⟨0x80⟩ : Ethereum.UInt256).toByteArray) := by
    exact Ethereum.UInt256.ofNat_toNat_of_lt (by omega)
  rw [hVal, toByteArray_128_eq_zeroes_append_128]
  rw [Ethereum.fromByteArrayBigEndian_append_singleton]
  change 128 ≤ 128 + 256 *
    Ethereum.fromByteArrayBigEndian (ffi.ByteArray.zeroes (⟨31⟩ : USize))
  omega
...
theorem mloadValue_mstoreNextState_same_eq
    (s : Ethereum.State) (a b : Ethereum.UInt256)
    (t : Ethereum.Stack Ethereum.UInt256)
    (hGuard :
      ¬ (a.toNat ≥ (mstoreNextState s a b t).machineState.memory.size ∨
        a ≥ (mstoreNextState s a b t).machineState.activeWords *
          (⟨32⟩ : Ethereum.UInt256)))
    (hDestPad : a.toNat - s.machineState.memory.size < USize.size) :
    mloadValue (mstoreNextState s a b t) a =
      Ethereum.UInt256.ofNat (Ethereum.fromByteArrayBigEndian b.toByteArray) := by
  unfold mloadValue
  rw [if_neg hGuard]
  rw [mstoreNextState_readWithPadding_word]
  exact hDestPad

theorem mloadValue_mstoreNextState_at_64_of_empty
    {s : Ethereum.State} {b : Ethereum.UInt256} {t : Ethereum.Stack Ethereum.UInt256}
    (hMemory : s.machineState.memory.size = 0)
    (hActiveWords : s.machineState.activeWords = (⟨0⟩ : Ethereum.UInt256)) :
    mloadValue
        (mstoreNextState s (⟨0x40⟩ : Ethereum.UInt256) b t)
        (⟨0x40⟩ : Ethereum.UInt256) =
      Ethereum.UInt256.ofNat (Ethereum.fromByteArrayBigEndian b.toByteArray) := by
  have hAddr : (⟨0x40⟩ : Ethereum.UInt256).toNat = 64 :=
    Ethereum.UInt256.toNat_fin_ofNat_of_lt (n := 0x40)
      (by norm_num [Ethereum.UInt256.size])
  have hDestPad : (⟨0x40⟩ : Ethereum.UInt256).toNat - s.machineState.memory.size <
      USize.size := by
    rw [hAddr, hMemory]
    change 64 < USize.size
    rcases System.Platform.numBits_eq with h | h <;> rw [USize.size, h] <;> norm_num
  apply mloadValue_mstoreNextState_same_eq
  · apply not_or.mpr
    constructor
    · have hSize :
          (⟨0x40⟩ : Ethereum.UInt256).toNat + 32 ≤
            (mstoreNextState s (⟨0x40⟩ : Ethereum.UInt256) b t).machineState.memory.size := by
        simpa [mstoreNextState] using
          ByteArray.write_size_ge_of_size
            (source := b.toByteArray)
            (dest := s.machineState.memory)
            (destAddr := (⟨0x40⟩ : Ethereum.UInt256).toNat)
            (len := 32)
            (by norm_num)
            (by simp)
            hDestPad
      omega
    · simp [mstoreNextState, hActiveWords, Ethereum.MachineState.M,
        Ethereum.UInt256.ofNat, Ethereum.UInt256.toNat, Id.run]
      decide
  · exact hDestPad

theorem mstoreNextState_at_64_of_empty_activeWords_eq_three
    {s : Ethereum.State} {b : Ethereum.UInt256} {t : Ethereum.Stack Ethereum.UInt256}
    (hActiveWords : s.machineState.activeWords = (⟨0⟩ : Ethereum.UInt256)) :
    (mstoreNextState s (⟨0x40⟩ : Ethereum.UInt256) b t).machineState.activeWords =
      (⟨3⟩ : Ethereum.UInt256) := by
  simp [mstoreNextState, hActiveWords, Ethereum.MachineState.M,
    Ethereum.UInt256.ofNat, Ethereum.UInt256.toNat, Id.run]
  rfl

theorem mloadNextState_at_64_of_activeWords_eq_three
    {s : Ethereum.State} {t : Ethereum.Stack Ethereum.UInt256}
    (hActiveWords : s.machineState.activeWords = (⟨3⟩ : Ethereum.UInt256)) :
    (mloadNextState s (⟨0x40⟩ : Ethereum.UInt256) t).machineState.activeWords =
      (⟨3⟩ : Ethereum.UInt256) := by
  simp [mloadNextState, hActiveWords, Ethereum.MachineState.M,
    Ethereum.UInt256.ofNat, Ethereum.UInt256.toNat, Id.run]
  rfl
...
theorem mstoreNextState_memory_size_ge_before
    (s : Ethereum.State) (a b : Ethereum.UInt256)
    (t : Ethereum.Stack Ethereum.UInt256) {addr len : Nat}
    (hLenPos : 0 < len)
    (hInBounds : addr + len ≤ s.machineState.memory.size)
    (hBefore : addr + len ≤ a.toNat) :
    addr + len ≤ (mstoreNextState s a b t).machineState.memory.size := by
  simpa [mstoreNextState] using
    ByteArray.write_size_ge_before_of_stop_le
      (source := b.toByteArray)
      (dest := s.machineState.memory)
      (sourceAddr := 0)
      (destAddr := a.toNat)
...
theorem mloadValue_mstoreNextState_before_eq_of_bounds
    (s : Ethereum.State) (ptr word a : Ethereum.UInt256)
    (t : Ethereum.Stack Ethereum.UInt256)
    (hBaseMemory : a.toNat + 32 ≤ s.machineState.memory.size)
    (hBaseActive :
      a.toNat < (s.machineState.activeWords * (⟨32⟩ : Ethereum.UInt256)).toNat)
    (hStoreMemory :
      a.toNat + 32 ≤ (mstoreNextState s ptr word t).machineState.memory.size)
    (hStoreActive :
      a.toNat <
        ((mstoreNextState s ptr word t).machineState.activeWords *
          (⟨32⟩ : Ethereum.UInt256)).toNat)
    (hBefore : a.toNat + 32 ≤ ptr.toNat) :
    mloadValue (mstoreNextState s ptr word t) a = mloadValue s a := by
  apply mloadValue_mstoreNextState_before_eq
  · apply not_or.mpr
    constructor
    · omega
    · intro hLe
      have hNat :
          (s.machineState.activeWords * (⟨32⟩ : Ethereum.UInt256)).toNat ≤
            a.toNat := hLe
      omega
  · apply not_or.mpr
    constructor
    ...
    · intro hLe
      have hNat :
          ((mstoreNextState s ptr word t).machineState.activeWords *
            (⟨32⟩ : Ethereum.UInt256)).toNat ≤ a.toNat := hLe
      omega
  · exact hBaseMemory
  · exact hBefore
...
lemma EVM_Xi_of_initial_continue_trace_return_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
...
lemma EVM_Xi_of_initial_continue_trace_add_oog_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
...
lemma EVM_Xi_of_initial_continue_trace_mload_memory_oog_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
...
lemma EVM_Xi_of_initial_continue_trace_mload_verylow_oog_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
...
lemma EVM_Xi_of_initial_continue_trace_dup1_oog_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
...
lemma EVM_Xi_of_initial_continue_trace_sub_oog_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
...
lemma EVM_Xi_of_initial_continue_trace_return_memory_oog_of_decode
    {createdAccounts : Batteries.RBSet Ethereum.AccountAddress compare}
    {genesisBlockHeader : Ethereum.BlockHeader}
    {blocks : Ethereum.ProcessedBlocks}
    {σ σ₀ : Ethereum.AccountMap}
