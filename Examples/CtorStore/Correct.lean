...
theorem ctorStoreGet?_append_left (A B : ByteArray) {i : Nat} (h : i < A.size) :
...
theorem ctorStoreExtract'_append_left (A B : ByteArray) {i j : Nat}
  ...
  exact extract_append_left A B i j hj

theorem ctorStoreDecode_append_left (A B : ByteArray) (pc : UInt256)
    (hpc : pc.toNat < A.size)
  ...
  unfold decode
  rw [ctorStoreGet?_append_left A B hpc]
  ...
  | some b =>
      ...
      | some instr =>
          ...
          · simp [harg]
          · simp [harg]
            rw [ctorStoreExtract'_append_left A B]
...
macro "ctor_store_decode_prefix" : tactic =>
  `(tactic|
    rw [ctorStoreDecode_append_left ctorStoreInitcode _ _ (by native_decide)
      (by
