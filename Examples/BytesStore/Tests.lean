...
/-!
# BytesStore — executable storage checks

These checks exercise the Solm storage semantics for the byte-write paths in
`BytesStore.sol`.  The Solidity source itself is compiled as part of the verification workflow;
...
def baseFrame : Frame :=
  { contract := bytesStoreContract, locals := ∅ }
...
def currentSetup : EvalResult (Frame × EVM.State) :=
  assignStorageRef? bytesStoreConfig baseFrame baseState .storage
    currentRef (.bytes sampleBytes)

def currentSetByteInBounds : EvalResult Value :=
  match currentSetup with
  | .ok (frame, evm) =>
      match assignStorageRef? bytesStoreConfig frame evm .storage
          (currentByteRef (.intLit 1)) (.int 0xaa) with
      | .ok (_, evm') => readStorage? bytesStoreConfig evm'
...
def currentSetByteOutOfBounds : EvalResult (Frame × EVM.State) :=
  match currentSetup with
  | .ok (frame, evm) =>
      assignStorageRef? bytesStoreConfig frame evm .storage
...
example (evm : EVM.State) :
    (bytesStoreStorageLayout.layout
      { base := "mapped", steps := [.mindex (.int 7), .length] } evm).isSome = true := by
  simp [bytesStoreStorageLayout, solidityStorageLayout, bytesStoreLayout,
    bytesLikeLengthLoc, Solm.bytesLikeLengthLoc]

def chunkSetup : EvalResult (Frame × EVM.State) :=
  match pushArray? bytesStoreConfig baseFrame baseState chunksRef
