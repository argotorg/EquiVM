import Examples.BytesStore.FullSetLongOldLong

/-!
# BytesStore — return wrappers for old-long clear long writes
-/
...
theorem bytesStore_shiftRight_add31_five_toNat_of_lt_sign {x : UInt256}
    (hx : x.toNat < 2 ^ 255) :
    (UInt256.shiftRight (x + ⟨31⟩) ⟨5⟩).toNat = (x.toNat + 31) / 32 := by
  rw [bytesStore_shiftRight_five_eq_div_thirtyTwo]
  exact StringStoreLite.u256_div_add31_toNat_of_lt_sign (x := x) hx

theorem bytesStore_shiftRight_add31_five_toNat_of_u64 {x : UInt256}
    (hx : x.toNat ≤ ABI.solcMaxU64) :
    (UInt256.shiftRight (x + ⟨31⟩) ⟨5⟩).toNat = (x.toNat + 31) / 32 := by
  rw [bytesStore_shiftRight_five_eq_div_thirtyTwo]
  exact StringStoreLite.u256_div_add31_toNat_of_u64 (x := x) hx

theorem bytesStoreX_setLongNoTailReturnsOldLongClearFromReach
  ...
  let τ : AccountMap := clearDataWordsForwardFrom I.codeOwner σ
    ...
    (UInt256.sub (UInt256.shiftRight (oldLen + ⟨31⟩) ⟨5⟩)
      (UInt256.shiftRight (len + ⟨31⟩) ⟨5⟩)).toNat
  have hbranch : ∃ k C, RD bytesStoreBytecode I g
...
theorem bytesStoreX_setLongTailReturnsOldLongClearFromReach
  ...
  have hbranch : ∃ k C, RD bytesStoreBytecode I g
    ...
    exact bytesStoreX_setLongOldLongClearReachWriteBranch
      ...
      (oldLen := oldLen)
      hperm hreach hnz hlong hlenMax hsrc hflag holdLen hvalid hgtOldNew
  exact bytesStoreX_setWriteLongTailReturnAfterClearBase
    (cA := cA) (gh := gh) (bl := bl) (σinit := σ) (τ := τ)
    (σ₀ := σ₀) (A := A) (I := I) (g := g) (len := len)
