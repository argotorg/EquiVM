...
theorem accountMapEquiv_pushChunkFinalWriteAfterClear {σ_evm σ_solm : AccountMap}
    (I : ExecutionEnv) (oldLen clearCount finalWord : UInt256) (oldFuel : Nat)
  ...
  simpa [hbaseZero, hcountNat] using
    accountMapEquiv_sstore_clearDataWordsForwardFrom I.codeOwner
      (bytesLikeDataBase (chunksDataBase + oldLen)) (⟨0⟩ : UInt256)
      (chunksDataBase + oldLen) finalWord oldFuel hlenAccounts

theorem bytesStoreX_bytesLengthDecoderLongValidMemCarried
...
theorem bytesStoreLongDataWordsLoopStride_zero_ofNat :
    ∀ i, StringStoreLite.longDataWordsLoopStride (⟨0⟩ : UInt256) i =
      UInt256.ofNat (32 * i)
  | 0 => rfl
  | i + 1 => by
      simp [StringStoreLite.longDataWordsLoopStride,
        bytesStoreLongDataWordsLoopStride_zero_ofNat i,
        StringStoreLite.u256_32_add_ofNat]
      congr 1

theorem bytesStorePushChunkLongPushArray_of_post_header {evm : EVM.State}
  ...
  have hwrite :
    ...
    exact bytesStoreWriteChunkLongPacked (evm := evmLen)
...
theorem bytesStorePushChunkLongPushArray_of_ne {evm : EVM.State}
    (oldLen header oldBytesLen : UInt256) (value : ByteArray)
    ...
    pushArray? bytesStoreConfig
      ...
      .ok (Solm.EVM.storageStore
        ...
        evm.executionEnv.codeOwner (chunksDataBase + oldLen)
        (solidityBytesHeaderWord value.size)) := by
  exact bytesStorePushChunkLongPushArray_of_post_header (evm := evm)
...
theorem bytesStorePushChunkLongOldLongPushArray_of_post_header {evm : EVM.State}
    (oldLen header oldBytesLen : UInt256) (value : ByteArray)
    (hvalueSize : ¬ value.size < 32)
    (hloadLen :
      Solm.EVM.storageLoad evm evm.executionEnv.codeOwner ⟨1⟩ = oldLen)
      ...
      writeStorage? bytesStoreConfig evmLen
        ...
        .ok (Solm.EVM.storageStore
          ...
          (writeSolidityBytesDataWordsFrom
            ...
            (solidityBytesDataWordCount value.size)).executionEnv.codeOwner
          (chunksDataBase + oldLen) (solidityBytesHeaderWord value.size)) := by
    exact bytesStoreWriteChunkLongFromLongPrepared (evm := evmLen)
      oldLen header oldBytesLen value hvalueSize
...
theorem bytesStorePushChunkLongOldLongPushArray_of_ne {evm : EVM.State}
    (oldLen header oldBytesLen : UInt256) (value : ByteArray)
    (hne : chunksDataBase + oldLen ≠ (⟨1⟩ : UInt256))
    (hvalueSize : ¬ value.size < 32)
    (hloadLen :
    ...
    pushArray? bytesStoreConfig
      ...
      .ok (Solm.EVM.storageStore
        ...
        (writeSolidityBytesDataWordsFrom
          ...
          (solidityBytesDataWordCount value.size)).executionEnv.codeOwner
        (chunksDataBase + oldLen) (solidityBytesHeaderWord value.size)) := by
  exact bytesStorePushChunkLongOldLongPushArray_of_post_header (evm := evm)
    oldLen header oldBytesLen value hvalueSize hloadLen
    (bytesStoreChunkHeaderAfterLengthStore_of_ne (evm := evm) oldLen header hne hloadElem)
    hflag hlen hvalid

theorem bytesStorePushChunkShortOldLongPushArray_of_post_header {evm : EVM.State}
    (oldLen header oldBytesLen : UInt256) (value : ByteArray)
    (hvalueSize : value.size < 32)
    (hloadLen :
      Solm.EVM.storageLoad evm evm.executionEnv.codeOwner ⟨1⟩ = oldLen)
  ...
  have hwrite :
      writeStorage? bytesStoreConfig evmLen
        ...
        .ok (Solm.EVM.storageStore
          ...
          (chunksDataBase + oldLen) (solidityShortBytesWord value)) := by
    exact bytesStoreWriteChunkShortFromLongPrepared (evm := evmLen)
      oldLen header oldBytesLen value hvalueSize
      (by simpa [evmLen, storageStore_executionEnv] using hloadElemLen) hflag hlen hvalid
  rw [pushArray?, bytesStoreChunksResolveValue evm value]
  simp only [bytesStoreConfig, bytesStoreStorageLayout, solidityStorageLayout,
...
theorem bytesStorePushChunkShortOldLongPushArray_of_ne {evm : EVM.State}
    (oldLen header oldBytesLen : UInt256) (value : ByteArray)
    (hne : chunksDataBase + oldLen ≠ (⟨1⟩ : UInt256))
    (hvalueSize : value.size < 32)
    (hloadLen :
    ...
    pushArray? bytesStoreConfig
      ...
      .ok (Solm.EVM.storageStore
        ...
        (clearSolidityBytesDataWordsFrom
          ...
          (chunksDataBase + oldLen) 0 ((oldBytesLen.toNat + 31) / 32)).executionEnv.codeOwner
        (chunksDataBase + oldLen) (solidityShortBytesWord value)) := by
  exact bytesStorePushChunkShortOldLongPushArray_of_post_header (evm := evm)
    oldLen header oldBytesLen value hvalueSize hloadLen
    (bytesStoreChunkHeaderAfterLengthStore_of_ne (evm := evm) oldLen header hne hloadElem)
    hflag hlen hvalid
...
theorem accountMapEquiv_pushChunkLongNoTailStorageFromLen
    ...
    accountMapEquiv
      (sstoreAccountMap I.codeOwner
        (bytesStoreCalldataLongDataForwardFrom I.codeOwner σ_evm_len
  ...
  have hbridgeEvm :
      accountMapEquiv
        (solidityDataWordsForwardFrom I.codeOwner σ_evm_len baseSlot
          (StringStoreLite.setDecodedValueBytes I) 0 (len.toNat / 32))
        (bytesStoreCalldataLongDataForwardFrom I.codeOwner σ_evm_len
          (bytesLikeDataBase baseSlot) payloadStart (⟨0⟩ : UInt256) I
          (len.toNat / 32)) := by
  ...
  have hcong :
      accountMapEquiv
        (solidityDataWordsForwardFrom I.codeOwner σ_evm_len baseSlot
          (StringStoreLite.setDecodedValueBytes I) 0 (len.toNat / 32))
        (solidityDataWordsForwardFrom I.codeOwner σ_solm_len baseSlot
          (StringStoreLite.setDecodedValueBytes I) 0 (len.toNat / 32)) := by
  ...
  simpa [baseSlot, hdataFuelEq, hheader] using
    accountMapEquiv_sstore_header_after_solidityDataWordsForwardFrom
      (owner := I.codeOwner) (σ := bytesStoreCalldataLongDataForwardFrom I.codeOwner
        σ_evm_len (bytesLikeDataBase baseSlot) payloadStart (⟨0⟩ : UInt256) I
        (len.toNat / 32))
      (τ := σ_solm_len) (baseSlot := baseSlot)
      (headerSlot := baseSlot) (header := len * (⟨2⟩ : UInt256) + ⟨1⟩)
      (bytes := StringStoreLite.setDecodedValueBytes I) (idx := 0)
      (fuel := len.toNat / 32) hdata
...
theorem accountMapEquiv_pushChunkLongTailStorageFromLen
      ...
      (sstoreAccountMap I.codeOwner
        (sstoreAccountMap I.codeOwner
          (bytesStoreCalldataLongDataForwardFrom I.codeOwner σ_evm_len
            (bytesLikeDataBase baseSlot) payloadStart (⟨0⟩ : UInt256) I
            (len.toNat / 32))
          (StringStoreLite.longDataWordsLoopSlot (bytesLikeDataBase baseSlot)
            (len.toNat / 32))
  ...
  have hstored :=
    accountMapEquiv_sstore_header_after_solidityDataWordsForwardFrom_succ_last
      (owner := I.codeOwner) (σ := bytesStoreCalldataLongDataForwardFrom I.codeOwner
        σ_evm_len (bytesLikeDataBase baseSlot) payloadStart (⟨0⟩ : UInt256) I fullFuel)
      (τ := σ_solm_len) (baseSlot := baseSlot)
...
theorem accountMapEquiv_pushChunkLongNoTailStorage
  ...
  have hlenAccounts : accountMapEquiv σ_evm_len σ_solm_len := by
    simpa [σ_evm_len, σ_solm_len, lenSlotVal] using
  ...
  have hdata :
      accountMapEquiv
        ...
        (solidityDataWordsForwardFrom I.codeOwner σ_solm_len baseSlot
          (StringStoreLite.setDecodedValueBytes I) 0 (len.toNat / 32)) :=
    accountMapEquiv.trans (accountMapEquiv.symm hbridgeEvm) hcong
  simpa [baseSlot, σ_evm_len, σ_solm_len, lenSlotVal, hdataFuelEq, hheader] using
    accountMapEquiv_sstore_header_after_solidityDataWordsForwardFrom
      (owner := I.codeOwner) (σ := bytesStoreCalldataLongDataForwardFrom I.codeOwner
        σ_evm_len (bytesLikeDataBase baseSlot) payloadStart (⟨0⟩ : UInt256) I
        (len.toNat / 32))
...
theorem bytesStoreX_pushChunkLongNoTailReachReturnWordSplit
    ...
    let oldLen : UInt256 := bytesStoreChunksLengthWord σ I
    let slot : UInt256 := chunksDataBase + oldLen
    let σLen : AccountMap := sstoreAccountMap I.codeOwner σ ⟨1⟩ (oldLen + ⟨1⟩)
    let σ' : AccountMap :=
      sstoreAccountMap I.codeOwner
        (bytesStoreCalldataLongDataForwardFrom I.codeOwner σLen
  ...
  let σ' : AccountMap :=
    sstoreAccountMap I.codeOwner
      (bytesStoreCalldataLongDataForwardFrom I.codeOwner σLen
        (bytesLikeDataBase slot) payloadStart (⟨0⟩ : UInt256) I (len.toNat / 32))
      slot (len * (⟨2⟩ : UInt256) + ⟨1⟩)
  have hbranch := bytesStoreX_pushChunkShortHeaderReachWriteBranch
    (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
    (g := g) (len := len) (payloadStart := payloadStart)
    hperm hreach hlenMax hflag hvalid
  have hwrite := bytesStoreX_writeBytesNewLongNoTailFromWriteBranch
    (cA := cA) (gh := gh) (bl := bl) (σinit := σ) (σ₀ := σ₀)
    (A := A) (I := I) (g := g)
    (τ := σLen) (slot := slot) (payloadStart := payloadStart) (len := len)
    (ret := ⟨1617⟩)
    (tail := [slot, ⟨0⟩, len, payloadStart, ⟨263⟩, bytesStoreSelWord I])
  ...
  have hret := bytesStoreX_pushChunkReturnFromHelperRetSplit
    (cA := cA) (gh := gh) (bl := bl) (σinit := σ)
    (σ := σ') (σ₀ := σ₀) (A := A) (I := I) (g := g)
    (slot := slot) (payloadStart := payloadStart) (len := len)
    (sel := bytesStoreSelWord I)
  ...
  simpa [oldLen, slot, σLen, σ', bytesStoreClearCurrentHashAw3] using hret

theorem bytesStoreWordAt0Mem_wordAt0Mem_read64
    (slot : UInt256) :
    (wordAt0Mem slot (wordAt0Mem (⟨1⟩ : UInt256) solcFreePtrMem)).readWithPadding 64 32 =
      UInt256.toByteArray ⟨128⟩ := by
  exact bytesStoreWordAt0Mem_read64 slot
    (wordAt0Mem_size_96 (⟨1⟩ : UInt256) solcFreePtrMem_size)
    (bytesStoreWordAt0Mem_read64 (⟨1⟩ : UInt256)
      solcFreePtrMem_size solcFreePtrMem_read64)

theorem bytesStoreWordAt0Mem_wordAt0Mem_wordAt0Mem_read64
    (slot : UInt256) :
  ...
  exact bytesStoreWordAt0Mem_read64 slot
    (wordAt0Mem_size_96 slot
      (wordAt0Mem_size_96 (⟨1⟩ : UInt256) solcFreePtrMem_size))
    (bytesStoreWordAt0Mem_wordAt0Mem_read64 slot)

theorem bytesStoreX_writeBytesCleanupOldLongLongToLoop {cA gh bl σinit σ σ₀ A I}
...
theorem bytesStoreX_writeBytesCleanupOldLongShortToLoop {cA gh bl σinit σ σ₀ A I}
    {g : Sat256} {slot oldLen len ret : UInt256} {tail : List UInt256}
    {mem rdata : ByteArray} {aw : UInt256}
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σinit σ₀ g A I) ⟨2302⟩
...
theorem bytesStoreX_pushChunkShortOldLongReachWriteBranchWithLoopSchedule
  ...
  have hdecoder := bytesStoreX_pushChunkReachWriteHeaderDecoder
...
theorem bytesStoreX_pushChunkShortOldLongReachWriteBranch
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  simpa [count] using
    bytesStoreX_pushChunkShortOldLongReachWriteBranchWithLoopSchedule
      (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A)
      (I := I) (g := g) (len := len) (payloadStart := payloadStart)
      (oldStoredLen := oldStoredLen) (fuel := count.toNat)
      hperm hreach hlenMax hflag holdStoredLen hvalid hshort hcontinue hdone

theorem bytesStoreX_pushChunkReturnFromShortWriteSplit {cA gh bl σinit σ σ₀ A I}
  ...
  exact bytesStoreX_pushChunkReturnFromHelperRetSplit
...
theorem bytesStoreX_pushChunkEmptyWriteHeaderGeneric {cA gh bl σinit σ σ₀ A I}
    {g : Sat256} {slot payloadStart len sel : UInt256}
    {mem rdata : ByteArray} {aw : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
...
theorem bytesStoreX_pushChunkShortOldLongReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  let storedWord : UInt256 :=
    UInt256.lor (UInt256.shiftLeft len ⟨1⟩)
      (UInt256.land
        ...
        (uInt256OfByteArray (I.calldata.readBytes payloadStart.toNat 32)))
  let σ' : AccountMap := sstoreAccountMap I.codeOwner σClear slot storedWord
  have hbranch := bytesStoreX_pushChunkShortOldLongReachWriteBranch
    (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
  ...
  have hwrite := bytesStoreX_writeBytesNewShortNonemptyWriteHeader
    ...
    (by simp only [List.length_cons, List.length_nil]; omega)
  have hret := bytesStoreX_pushChunkReturnFromShortWriteSplit
    (cA := cA) (gh := gh) (bl := bl) (σinit := σ)
    (σ := σ') (σ₀ := σ₀) (A := A) (I := I) (g := g)
    (slot := slot) (payloadStart := payloadStart) (len := len)
    (sel := bytesStoreSelWord I)
...
theorem bytesStoreX_pushChunkShortOldLongReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σClear, storedWord, σ'] using
        bytesStoreX_pushChunkShortOldLongReachReturnWordSplit
...
theorem bytesStoreX_pushChunkEmptyOldLongReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  have hpacked := bytesStoreX_writeBytesNewEmptyReachPackedHeader
    ...
    hlenZero
    (by simp only [List.length_cons, List.length_nil]; omega)
  have hwrite := bytesStoreX_pushChunkEmptyWriteHeaderGeneric
    (cA := cA) (gh := gh) (bl := bl) (σinit := σ) (σ := σClear) (σ₀ := σ₀)
    (A := A) (I := I) (g := g) (slot := slot) (payloadStart := payloadStart)
    (len := len) (sel := bytesStoreSelWord I)
    (mem := wordAt0Mem slot (wordAt0Mem (⟨1⟩ : UInt256) solcFreePtrMem))
    (aw := StringStoreLite.clearCurrentHashAw (UInt256.ofNat 3))
    (rdata := ByteArray.empty)
    hperm
...
theorem bytesStoreX_pushChunkEmptyOldLongReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σClear, σ'] using
        bytesStoreX_pushChunkEmptyOldLongReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          (oldStoredLen := oldStoredLen)
          hperm hreach hlenMax hflag holdStoredLen hvalid hlenZero)

theorem bytesStoreX_pushChunkLongHeaderNoClearReachWriteBranch
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
...
theorem bytesStoreX_pushChunkLongHeaderClearReachWriteBranchWithLoopSchedule
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    {fuel : Nat}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
...
theorem bytesStoreX_pushChunkLongHeaderClearReachWriteBranch
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  have hdone :
    ...
    exact ult_zero (a := count) (b := count) (Nat.le_refl _)
  simpa [count] using
    bytesStoreX_pushChunkLongHeaderClearReachWriteBranchWithLoopSchedule
      (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A)
      (I := I) (g := g) (len := len) (payloadStart := payloadStart)
      (oldStoredLen := oldStoredLen) (fuel := count.toNat)
...
theorem bytesStoreX_pushChunkLongNoTailOldLongClearReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  let σ' : AccountMap :=
    sstoreAccountMap I.codeOwner
      (bytesStoreCalldataLongDataForwardFrom I.codeOwner σClear
        (bytesLikeDataBase slot) payloadStart (⟨0⟩ : UInt256) I (len.toNat / 32))
      slot (len * (⟨2⟩ : UInt256) + ⟨1⟩)
  have hbranch := bytesStoreX_pushChunkLongHeaderClearReachWriteBranch
    (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
    (g := g) (len := len) (payloadStart := payloadStart) (oldStoredLen := oldStoredLen)
    hperm hreach hlenMax hflag holdStoredLen hvalid hgtOldNew hlong
  have hwrite := bytesStoreX_writeBytesNewLongNoTailFromWriteBranch
  ...
  have hret := bytesStoreX_pushChunkReturnFromHelperRetSplit
    ...
    (rdata := ByteArray.empty)
    (by simpa [oldLen, slot, σLen, σClear, σ'] using hwrite)
  simpa [oldLen, slot, σLen, σClear, σ', bytesStoreClearCurrentHashAw3] using hret

theorem bytesStoreX_pushChunkLongNoTailOldLongClearReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (wordAt0Mem_size_96 slot
      (wordAt0Mem_size_96 slot
        (wordAt0Mem_size_96 (⟨1⟩ : UInt256) solcFreePtrMem_size)))
    (bytesStoreWordAt0Mem_wordAt0Mem_wordAt0Mem_read64 slot)
    (by
      simpa [oldLen, slot, σLen, σClear, σ'] using
        bytesStoreX_pushChunkLongNoTailOldLongClearReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          (oldStoredLen := oldStoredLen)
          hperm hreach hlenMax hflag holdStoredLen hvalid hgtOldNew hlong hnoTailMod)

theorem bytesStoreX_pushChunkLongTailOldLongClearReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  have hbranch := bytesStoreX_pushChunkLongHeaderClearReachWriteBranch
    ...
    (g := g) (len := len) (payloadStart := payloadStart) (oldStoredLen := oldStoredLen)
    hperm hreach hlenMax hflag holdStoredLen hvalid hgtOldNew hlong
  have hwrite := bytesStoreX_writeBytesNewLongTailFromWriteBranchSplit
    (cA := cA) (gh := gh) (bl := bl) (σinit := σ) (σ₀ := σ₀)
    (A := A) (I := I) (g := g)
    (τ := σClear) (slot := slot) (payloadStart := payloadStart) (len := len)
    (ret := ⟨1617⟩)
...
theorem bytesStoreX_pushChunkLongTailOldLongClearReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σClear, σ'] using
        bytesStoreX_pushChunkLongTailOldLongClearReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          (oldStoredLen := oldStoredLen)
          hperm hreach hlenMax hflag holdStoredLen hvalid hgtOldNew hlong htailMod)

theorem bytesStoreX_pushChunkLongNoTailOldLongNoClearReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  let σ' : AccountMap :=
    sstoreAccountMap I.codeOwner
      (bytesStoreCalldataLongDataForwardFrom I.codeOwner σLen
        (bytesLikeDataBase slot) payloadStart (⟨0⟩ : UInt256) I (len.toNat / 32))
      slot (len * (⟨2⟩ : UInt256) + ⟨1⟩)
  have hbranch := bytesStoreX_pushChunkLongHeaderNoClearReachWriteBranch
    (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
    (g := g) (len := len) (payloadStart := payloadStart) (oldStoredLen := oldStoredLen)
...
theorem bytesStoreX_pushChunkLongNoTailReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σ'] using
        bytesStoreX_pushChunkLongNoTailReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          hperm hreach hlenMax hflag hvalid hlong hnoTailMod)

theorem bytesStoreX_pushChunkLongNoTailOldLongNoClearReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σ'] using
        bytesStoreX_pushChunkLongNoTailOldLongNoClearReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          (oldStoredLen := oldStoredLen)
          hperm hreach hlenMax hflag holdStoredLen hvalid hgtOldNew hlong hnoTailMod)

theorem bytesStoreX_pushChunkLongTailReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
...
theorem bytesStoreX_pushChunkLongTailOldLongNoClearReachReturnWordSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
...
theorem bytesStoreX_pushChunkLongTailReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σ'] using
        bytesStoreX_pushChunkLongTailReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          hperm hreach hlenMax hflag hvalid hlong htailMod)

theorem bytesStoreX_pushChunkLongTailOldLongNoClearReturnsSplit
    {cA gh bl σ σ₀ A I} {g : Sat256} {len payloadStart oldStoredLen : UInt256}
    (hperm : I.perm = true)
    (hreach : ∃ k C, RD bytesStoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨1559⟩
  ...
  exact bytesStoreX_returnWord263OfMemState
    ...
    (by
      simpa [oldLen, slot, σLen, σ'] using
        bytesStoreX_pushChunkLongTailOldLongNoClearReachReturnWordSplit
          (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀)
          (A := A) (I := I) (g := g) (len := len) (payloadStart := payloadStart)
          (oldStoredLen := oldStoredLen)
          hperm hreach hlenMax hflag holdStoredLen hvalid hgtOldNew hlong htailMod)
