...
/-! ### A local width-generic selector arm for `balanceOf`'s `PUSH3` selector -/

-- LIBRARY CANDIDATE: `Reasoning.Reach`.
-- Generalizes `selectorArmTakenAuto`/`selectorArmNotTakenAuto` by parameterizing the selector push
-- width.  Solc can encode selectors with leading zero bytes using `PUSH3`, as in ERC6909
