...
/-!
# BytesStoreLite — focused bytes storage example
...
def bytesStoreLiteContract : ContractDecl :=
  { name := "BytesStoreLite"
...
def bytesStoreLiteLayout : EvaledStorageRef -> EVM.State -> Option StorageLoc
...
def bytesStoreLiteStorageLayout : StorageLayout :=
  solidityStorageLayout bytesStoreLiteLayout

end BytesStoreLite

def bytesStoreLiteConfig : Config :=
  { storage := BytesStoreLite.bytesStoreLiteStorageLayout
    externalABI := defaultExternalCallABI
    selfDeployment :=
      genSolidityConstructorDeployment BytesStoreLite.bytesStoreLiteContract.ctor.params }
