...
abbrev bytesStoreLiteOptimizedLongTailMaskedWordLocal (word len : UInt256) : UInt256 :=
  UInt256.land
    (UInt256.lnot
...
theorem bytesStoreLiteOptimizedLongTailMaskedWordLocal_eq_core
    (word len : UInt256) :
    bytesStoreLiteOptimizedLongTailMaskedWordLocal word len =
      BytesStoreLiteCore.longDataTailMaskedWord word len := by
  exact bytesStoreLiteOptimizedLongTailMaskedWord_eq_core word len

theorem bytesStoreLiteOptimizedLongTailMaskedWordRaw_eq_core
    (word len : UInt256) :
    UInt256.land word
      (UInt256.lnot
        (UInt256.shiftRight (UInt256.lnot ⟨0⟩)
...
theorem bytesStoreLiteX_writeBytesNewLongTailStoreFrom2749Split
...
theorem bytesStoreLiteX_writeBytesNewLongTailStoreFrom2749CoreSplit
    {cA gh bl σinit σ₀ A I} {g : Sat256} {τ : AccountMap}
    {slot cutoff gtFlag stride baseSlot payloadStart len ret : UInt256}
    {tail : List UInt256} {mem rdata : ByteArray} {aw : UInt256}
    (hperm : I.perm = true)
      ...
      RD bytesStoreLiteBytecode I g
        ...
        (cA, sstoreAccountMap I.codeOwner τ slot
          (BytesStoreLiteCore.longDataTailMaskedWord
            (bytesStoreLiteCalldataLongDataWord I payloadStart stride 0) len)) k' C' := by
  obtain ⟨_, _, htail⟩ :=
    bytesStoreLiteX_writeBytesNewLongTailStoreFrom2749Split
      (hperm := hperm) (hreach := hreach) (hov := hov)
  exact ⟨_, _, by
    simpa [bytesStoreLiteCalldataLongDataWord,
      BytesStoreLiteCore.longDataWordsLoopStride,
      bytesStoreLiteOptimizedLongTailMaskedWordRaw_eq_core]
      using htail⟩

theorem bytesStoreLiteX_writeBytesNewLongDataWordsLoopDoneTailSplit
    {cA gh bl σinit σ₀ A I} {g : Sat256} {τ : AccountMap}
      ...
      RD bytesStoreLiteBytecode I g
        ...
        (cA, sstoreAccountMap I.codeOwner
          (sstoreAccountMap I.codeOwner τ slot
            (BytesStoreLiteCore.longDataTailMaskedWord
              (bytesStoreLiteCalldataLongDataWord I payloadStart stride 0) len))
  ...
  have rd2749 := evm_run rd2739 with [
    ...
    jumpiNT htail]
  have htailStore := bytesStoreLiteX_writeBytesNewLongTailStoreFrom2749CoreSplit
...
theorem bytesStoreLiteX_writeBytesNewLongTailFromWriteBranchSplit
    {cA gh bl σinit σ₀ A I} {g : Sat256} {τ : AccountMap}
  ...
  have hloop := bytesStoreLiteX_writeBytesNewLongDataWordsLoopGenerated
    ...
    hov
  exact bytesStoreLiteX_writeBytesNewLongDataWordsLoopDoneTailSplit
    (cA := cA) (gh := gh) (bl := bl) (σinit := σinit) (σ₀ := σ₀)
    (A := A) (I := I) (g := g)
...
theorem bytesStoreLiteClearCurrentHashAw3 :
    BytesStoreLiteCore.clearCurrentHashAw (UInt256.ofNat 3) = UInt256.ofNat 3 := by
  native_decide

theorem bytesStoreLiteX_pushChunkReturnFromHelperRetSplit
