...
import Examples.BytesStoreLite.Constructor
...
import Examples.BytesStoreLite.FullPushChunkLongRuntime
import Examples.BytesStoreLite.FullSetLongOldLongReturn
import Examples.BytesStoreLite.FullSetLongOldLongRuntime
import Examples.BytesStoreLite.FullSetPacketRuntime
import Examples.BytesStoreLite.FullSetChunkOldLongReturn
import Examples.BytesStoreLite.FullSetChunkByteSuccess
import Examples.BytesStoreLite.FullSetMapped
import Examples.BytesStoreLite.FullSetMappedByteSuccess
import Examples.BytesStoreLite.FullClearCurrentLong
import Examples.BytesStoreLite.CoreCorrect
import Examples.BytesStoreLite.Tests

open Solm ABI Ethereum Ethereum.EVM Reasoning.Theory

/-!
# BytesStoreLite — top-level example module

This module keeps the bytes-focused Solidity/spec/test example available from `Examples.lean`.
`Bytecode.lean` pins the optimized full-contract bytecode for `BytesStoreLite.sol`; `Constructor.lean`
...
The proved whole-contract target is still `BytesStoreLiteCore`, exposed by
`BytesStoreLiteCore.bytesStoreLiteCoreCorrect`; it covers `set(bytes)`, `clearCurrent()`, and
`currentLength()` over a single dynamic byte array.  The full `BytesStoreLite` constructor,
no-dispatch cases, five full-runtime getter branches, and the complete full-runtime
`clearCurrent()` selector arm are pinned/proved here as scaffolding for the remaining runtime
equivalence work, plus the malformed `pushChunk(bytes)` decoder failure branches
...
theorem bytesStoreLiteCorrect :
    runtimeEquivalence!?! bytesStoreLiteConfig bytesStoreLiteBytecode
      bytesStoreLiteContract := by
    ...
    · by_cases hsz : 4 ≤ I.calldata.size
      · by_cases hselSet : selIs I ⟨#[0x03, 0x99, 0x32, 0x1e]⟩
        · exact bytesStoreLiteSetRuntime hcode hsize hperm hwv hselSet hAccounts
        · by_cases hselSetByte : selIs I ⟨#[0x1c, 0x52, 0x47, 0x7d]⟩
          · exact bytesStoreLiteSetByteRuntime hcode hsize hperm hwv hselSetByte hAccounts
          · by_cases hselClear : selIs I ⟨#[0xa6, 0xdf, 0xa2, 0x62]⟩
            · exact bytesStoreLiteClearCurrentRuntime hcode hsize hperm hwv hselClear hAccounts
            · by_cases hselCurrent : selIs I ⟨#[0xa3, 0xd3, 0x5f, 0x36]⟩
              · exact bytesStoreLiteCurrentLengthRuntime hcode hsize hperm hwv hselCurrent hAccounts
              · by_cases hselPush : selIs I ⟨#[0xb5, 0x0c, 0xc8, 0xac]⟩
                · exact bytesStoreLitePushChunkRuntime hcode hsize hperm hwv hselPush hAccounts
                · by_cases hselSetChunk : selIs I ⟨#[0x39, 0x3d, 0x9c, 0xd7]⟩
                  · exact bytesStoreLiteSetChunkRuntime hcode hsize hperm hwv
                      hselSetChunk hAccounts
                  · by_cases hselSetChunkByte : selIs I ⟨#[0x48, 0x1c, 0x2f, 0xfb]⟩
                    · exact bytesStoreLiteSetChunkByteRuntime hcode hsize hperm hwv
                        hselSetChunkByte hAccounts
                    · by_cases hselChunkLength : selIs I ⟨#[0xe8, 0xbd, 0x9a, 0xf5]⟩
                      · exact bytesStoreLiteChunkLengthRuntime hcode hsize hperm hwv
                          hselChunkLength hAccounts
                      · by_cases hselSetPacket : selIs I ⟨#[0x14, 0xf1, 0x7c, 0x69]⟩
                        · exact bytesStoreLiteSetPacketRuntime hcode hsize hperm hwv
                            hselSetPacket hAccounts
                        · by_cases hselSetPacketByte : selIs I ⟨#[0x0a, 0xb2, 0x59, 0x00]⟩
                          · exact bytesStoreLiteSetPacketByteRuntime hcode hsize hperm hwv
                              hselSetPacketByte hAccounts
                          · by_cases hselPacketLength : selIs I ⟨#[0xb5, 0x18, 0xd2, 0xc4]⟩
                            · exact bytesStoreLitePacketLengthRuntime hcode hsize hperm hwv
                                hselPacketLength hAccounts
                            · by_cases hselPacketTag : selIs I ⟨#[0x99, 0x3e, 0x0a, 0x90]⟩
                              · exact bytesStoreLitePacketTagRuntime hcode hsize hperm hwv
                                  hselPacketTag hAccounts
                              · by_cases hselSetMapped : selIs I ⟨#[0xe1, 0x91, 0x9b, 0x17]⟩
                                · exact bytesStoreLiteSetMappedRuntime hcode hsize hperm hwv
                                    hselSetMapped hAccounts
                                · by_cases hselSetMappedByte :
                                    selIs I ⟨#[0x13, 0x2f, 0xa3, 0x46]⟩
                                  · exact bytesStoreLiteSetMappedByteRuntime hcode hsize hperm hwv
                                      hselSetMappedByte hAccounts
                                  · by_cases hselMappedLength :
                                      selIs I ⟨#[0x39, 0x1d, 0x72, 0x80]⟩
                                    · exact bytesStoreLiteMappedLengthRuntime hcode hsize hperm hwv
                                        hselMappedLength hAccounts
                                    · refine bytesStoreLiteNoDispatch hcode hsize ?_
                                      intro i hi
                                      interval_cases i
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSet)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetByte)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselClear)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselCurrent)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselPush)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetChunk)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetChunkByte)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselChunkLength)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetPacket)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetPacketByte)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselPacketLength)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselPacketTag)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetMapped)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselSetMappedByte)
                                      · exact Bool.eq_false_of_not_eq_true
                                          (by simpa [selIs, bytesStoreLiteSelBytes] using hselMappedLength)
