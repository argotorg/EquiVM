...
/-!
# BytesStoreLiteCore — `currentLength()` long-string branch work
...
def CurrentLengthLoopState.stack (s : CurrentLengthLoopState) (endp len : UInt256)
  ...
  [s.ptr, s.slot, endp, len, ⟨0⟩, ⟨128⟩, ⟨0⟩, ⟨0⟩, ⟨153⟩, bytesStoreLiteCoreSelWord I]
...
structure CurrentLengthLoopFinal (σ : AccountMap) (I : ExecutionEnv) (endp len : UInt256)
  ...
  hmloadCost : ∀ st : State, st.machineState.activeWords = awStore →
    st.machineState.stack = [⟨128⟩, ⟨128⟩, ⟨0⟩, ⟨153⟩, bytesStoreLiteCoreSelWord I] →
...
theorem bytesStoreLiteCoreX_clearCurrentLongReachCopyLoop {cA gh bl σ σ₀ A I} {g : Sat256}
    {len : UInt256}
...
theorem bytesStoreLiteCoreX_clearCurrentLongFinalCopyToDelete {cA gh bl σ σ₀ A I}
    ...
    (hmloadCost : ∀ s : State, s.machineState.activeWords = awStore →
      s.machineState.stack = [⟨128⟩, ⟨128⟩, ⟨0⟩, ⟨153⟩, bytesStoreLiteCoreSelWord I] →
  ...
  obtain ⟨_, _, rd400⟩ : ∃ k C, RD bytesStoreLiteCoreBytecode I g
...
theorem bytesStoreLiteCoreX_clearCurrentLongCopyContinue {cA gh bl σ σ₀ A I}
...
theorem bytesStoreLiteCoreX_clearCurrentLongCopyLoopSchedule {cA gh bl σ σ₀ A I}
  ...
  | zero =>
      simpa [CurrentLengthLoopState.stack] using
        bytesStoreLiteCoreX_clearCurrentLongFinalCopyToDelete
  ...
  | succ fuel ih =>
      have hs : CurrentLengthLoopStep σ I endp len (st 0) (st 1) := by
        simpa using hsteps 0 (Nat.zero_lt_succ fuel)
      have hnext₀ := bytesStoreLiteCoreX_clearCurrentLongCopyContinue
        (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
        (g := g) (ptr := (st 0).ptr) (slot := (st 0).slot) (endp := endp)
        (len := len) (aw := (st 0).aw) (m := (st 0).mem)
      ...
      have hnext : ∃ k C, RD bytesStoreLiteCoreBytecode I g
          (initState cA gh bl σ σ₀ g A I) ⟨397⟩
...
theorem bytesStoreLiteCoreX_clearCurrentLongReachDeleteWithSchedule
    ...
    ∃ k C, RD bytesStoreLiteCoreBytecode I g
      ...
      [⟨0⟩, ⟨0⟩, ⟨449⟩, ⟨128⟩, len, ⟨153⟩, bytesStoreLiteCoreSelWord I]
      hfinal.memout hfinal.awLoad ByteArray.empty (cA, σ) k C := by
  have hdecoded₀ := bytesStoreLiteCoreX_bytesLengthDecoderLongValid
  ...
  have hdecodedCopy := bytesStoreLiteCoreX_bytesLengthDecoderLongValidMem
  ...
  have hloop := bytesStoreLiteCoreX_clearCurrentLongReachCopyLoop
    (g := g) ⟨_, _, rd351⟩ hnonzero hgt31
  exact bytesStoreLiteCoreX_clearCurrentLongCopyLoopSchedule
    (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
    (g := g) (endp := (⟨160⟩ : UInt256) + len) (len := len) fuel st
    hsteps hfinal (by
      simpa [CurrentLengthLoopState.stack, hinit] using hloop)

theorem bytesStoreLiteCoreX_clearCurrentLongReachDeleteGenerated
    {cA gh bl σ σ₀ A I} {g : Sat256} {len : UInt256}
    (hreach : ∃ k C, RD bytesStoreLiteCoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨145⟩ [bytesStoreLiteCoreSelWord I]
      solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C)
        ...
        (by
          ...
          simpa [finalMloadCost, awStore] using
            (mloadCostSpec
              ...
              (stk := [⟨128⟩, ⟨0⟩, ⟨153⟩, bytesStoreLiteCoreSelWord I]) st haw hstk))
  ...
  have hawLoad :
    ...
    rfl
  exact bytesStoreLiteCoreX_clearCurrentLongReachDeleteWithSchedule
    (cA := cA) (gh := gh) (bl := bl) (σ := σ) (σ₀ := σ₀) (A := A) (I := I)
...
theorem bytesStoreLiteCoreX_clearCurrentReturnFromWrapperGeneric
    ...
    (hreach : ∃ k C, RD bytesStoreLiteCoreBytecode I g
      ...
      [len, bytesStoreLiteCoreSelWord I] mem aw rdata (cA, τ) k C)
    (hmloadCost : ∀ s : State, s.machineState.activeWords = aw →
      s.machineState.stack = [⟨64⟩, len, bytesStoreLiteCoreSelWord I] →
      memoryExpansionCost s .MLOAD = mloadCost)
    ...
    (hmstoreCost : ∀ s : State, s.machineState.activeWords = awLoad →
      s.machineState.stack =
        [freePtr + ⟨0⟩, len, len, freePtr + ⟨0⟩, ⟨763⟩, freePtr + ⟨32⟩,
          freePtr, len, ⟨166⟩, bytesStoreLiteCoreSelWord I] →
      memoryExpansionCost s .MSTORE = mstoreCost)
    ...
    (hfinalMloadCost : ∀ s : State, s.machineState.activeWords = awStore →
      s.machineState.stack = [⟨64⟩, freePtr + ⟨32⟩, bytesStoreLiteCoreSelWord I] →
    ...
    (hretCost : ∀ s : State, s.machineState.activeWords = awFinal →
      s.machineState.stack =
        [freePtr, UInt256.sub (freePtr + ⟨32⟩) freePtr, bytesStoreLiteCoreSelWord I] →
      memoryExpansionCost s .RETURN = retCost) :
    RDret bytesStoreLiteCoreBytecode g (initState cA gh bl σinit σ₀ g A I) (cA, τ)
      (UInt256.toByteArray len) := by
...
theorem bytesStoreLiteCoreX_clearCurrentLongValidGenerated {cA gh bl σ σ₀ A I}
  ...
  have hreadStart : ∃ k C, RD bytesStoreLiteCoreBytecode I g
      (initState cA gh bl σ σ₀ g A I) ⟨453⟩
      [⟨0⟩, ⟨0⟩, ⟨449⟩, ⟨128⟩, len, ⟨153⟩, bytesStoreLiteCoreSelWord I]
    ...
    simpa [copyFinal, fuel, copyAwStore, copyMloadCost, copyAwLoad] using
      bytesStoreLiteCoreX_clearCurrentLongReachDeleteGenerated
        (g := g) (len := len) hreach hflag hvalid hlen hnonzero hgt31
  have hdelReach := bytesStoreLiteCoreX_clearCurrentDeleteLongValid
    ...
    simpa [wrapperMstoreCost, wrapperAwStore, currentLength_add_zero_toNat] using
      (mstoreCostSpec (aw := deleteAw) (off := freePtr + ⟨0⟩)
        (stk := [len, len, freePtr + ⟨0⟩, ⟨763⟩, freePtr + ⟨32⟩, freePtr, len,
          ⟨166⟩, bytesStoreLiteCoreSelWord I]) s haw hstk)
  ...
  exact bytesStoreLiteCoreX_clearCurrentReturnFromWrapperGeneric
    ...
    (by
      ...
      simpa [wrapperMloadCost] using
        (mloadCostSpec (aw := deleteAw) (off := (⟨64⟩ : UInt256))
          (stk := [len, bytesStoreLiteCoreSelWord I]) s haw hstk))
    hfreePtrVal
    hwrapperAwLoad
    hreturnMem
    hmstoreCost
    ...
    (by
      ...
      simpa [wrapperFinalMloadCost, wrapperAwFinal] using
        (mloadCostSpec (aw := wrapperAwStore) (off := (⟨64⟩ : UInt256))
          (stk := [freePtr + ⟨32⟩, bytesStoreLiteCoreSelWord I]) s haw hstk))
    hfinalFreePtrVal
    hwrapperAwFinal
    hretBytes
    hretCost
