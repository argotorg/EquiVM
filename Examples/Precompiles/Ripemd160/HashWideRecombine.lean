import Examples.Precompiles.Ripemd160.HashWideInitInvariant
...
theorem ripemd160X_recombine_wideGas {cA σ I} {g : Sat256} {s0 : State}
  ...
  have awEq (m : UInt256) (hm : m.toNat ≤ 800) :
    ...
    · rw [addrNat m (by omega)]
      omega
  have costEq (m : UInt256) (hm : m.toNat ≤ 800) :
      runtimeMloadCost c.aw (hashScratchPtr I + m) = 0 := by
    simp only [runtimeMloadCost, awEq m hm, Nat.sub_self]
...
theorem ripemd160X_recombine_wide {cA σ I} {g : Sat256} {s0 : State}
  ...
  obtain ⟨k', h'⟩ := ripemd160X_recombine_wideGas hinv hsmall rd1406
  exact ⟨k', _, h'⟩
