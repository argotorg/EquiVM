...
/-!
# BLAKE2F positive-round exact stepping support

Local exact-gas support for opcodes needed by the BLAKE2F positive-round trace but not currently
exported by `Reasoning.ReachExact`.

The `DUP12` one-step lemma and `RDx` bridge are copied from the RIPEMD-160 support files:
`Examples/Precompiles/Ripemd160/ProofSupport.lean` and
`Examples/Precompiles/Ripemd160/ProofSupportExact.lean`.
-/
