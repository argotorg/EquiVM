...
/-!
# BLAKE2F positive-round wrappers: round-1 completion

Context-level wrappers after round 1 has completed and control has returned to the rounds-loop
guard with loop index `2`.
-/
...
/-- Valid final-flag-`0` inputs with at least two rounds have completed round 1 and returned to
the rounds-loop guard.

The cursor is PC `1370` with loop index `2`. Exact cumulative gas is `14461`. -/
theorem validPositiveRound1DoneZeroFlagPrefix
    (ctx : BytecodeContext)
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound1DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least two rounds have completed round 1 and returned to
the rounds-loop guard.

The cursor is PC `1370` with loop index `2`. Exact cumulative gas is `14488`. -/
theorem validPositiveRound1DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
  ...
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound1DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
