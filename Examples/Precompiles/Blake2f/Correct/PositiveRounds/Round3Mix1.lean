...
/-!
# BLAKE2F positive-round wrappers: round-3 second `mixG`

Context-level wrappers for the completed second round-3 `mixG` call on valid inputs with at least
four rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least four rounds have completed the second round-3
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-3 message arguments `m[3]` and `m[1]`.
Exact cumulative gas is `18571 + 321 = 18892`. -/
theorem validPositiveRound3Mix1DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round3Mix1Mem3 (round3Mix0ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 18892 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix1DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least four rounds have completed the second round-3
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-3 message arguments `m[3]` and `m[1]`.
Exact cumulative gas is `18598 + 321 = 18919`. -/
theorem validPositiveRound3Mix1DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round3Mix1Mem3 (round3Mix0OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 18919 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix1DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
