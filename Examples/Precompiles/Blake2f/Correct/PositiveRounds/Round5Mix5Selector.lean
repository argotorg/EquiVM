...
/-!
# BLAKE2F positive-round wrappers: round-5 second diagonal `mixG` argument setup

Context-level wrappers for the second diagonal round-5 `mixG` argument setup on the path with at
least six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have prepared the second diagonal
round-5 `mixG` call.

The cursor is PC `2656`, with packed round-5 SIGMA word `0x2c6a0b834d75fe19` and message
arguments `m[7]`/`m[5]`. Exact cumulative gas is `27128 + 93 = 27221`. -/
theorem validPositiveRound5Mix5ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2656⟩
      [round5M5Arg (round5Mix4ZeroMem ctx.executionEnv),
        round5M7Arg (round5Mix4ZeroMem ctx.executionEnv),
        ⟨2840⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix4ZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 27221 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix5ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have prepared the second diagonal
round-5 `mixG` call.

The cursor is PC `2656`, with packed round-5 SIGMA word `0x2c6a0b834d75fe19` and message
arguments `m[7]`/`m[5]`. Exact cumulative gas is `27155 + 93 = 27248`. -/
theorem validPositiveRound5Mix5ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2656⟩
      [round5M5Arg (round5Mix4OneMem ctx.executionEnv),
        round5M7Arg (round5Mix4OneMem ctx.executionEnv),
        ⟨2840⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix4OneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 27248 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix5ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
