...
/-!
# BLAKE2F positive-round wrappers: round-5 fourth diagonal `mixG`

Context-level wrappers for the completed fourth diagonal round-5 `mixG` call on the path with at
least six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have completed all eight round-5
`mixG` calls.

The cursor is PC `3292`, immediately after updating bytecode vector slots `v[3]`, `v[4]`,
`v[9]`, and `v[14]` with round-5 message arguments `m[1]` and `m[9]`.
Exact cumulative gas is `28040 + 318 = 28358`. -/
theorem validPositiveRound5Mix7DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3292⟩
      [⟨5⟩, ⟨1⟩, ⟨640⟩, UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩,
        ⟨384⟩, ⟨1472⟩, ⟨168⟩, ⟨1216⟩]
      (round5Mix7Mem3 (round5Mix6ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28358 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix7DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have completed all eight round-5
`mixG` calls.

The cursor is PC `3292`, immediately after updating bytecode vector slots `v[3]`, `v[4]`,
`v[9]`, and `v[14]` with round-5 message arguments `m[1]` and `m[9]`.
Exact cumulative gas is `28067 + 318 = 28385`. -/
theorem validPositiveRound5Mix7DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [⟨5⟩, ⟨1⟩, ⟨640⟩, UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩,
        ⟨384⟩, ⟨1472⟩, ⟨168⟩, ⟨1216⟩]
      (round5Mix7Mem3 (round5Mix6OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28385 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix7DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
