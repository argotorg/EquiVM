...
/-!
# BLAKE2F positive-round wrappers: round-3 first `mixG`

Context-level wrappers for the completed first round-3 `mixG` call on valid inputs with at least
four rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least four rounds have completed the first round-3
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-3 message arguments `m[7]` and `m[9]`.
Exact cumulative gas is `18163 + 315 = 18478`. -/
theorem validPositiveRound3Mix0DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round3Mix0Mem3 (round2DoneZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 18478 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix0DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least four rounds have completed the first round-3
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-3 message arguments `m[7]` and `m[9]`.
Exact cumulative gas is `18190 + 315 = 18505`. -/
theorem validPositiveRound3Mix0DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round3Mix0Mem3 (round2DoneOneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 18505 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix0DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
