...
/-!
# BLAKE2F positive-round wrappers: round-5 first `mixG`

Context-level wrappers for the completed first round-5 `mixG` call on valid inputs with at least
six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have completed the first round-5
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-5 message arguments `m[2]` and `m[12]`.
Exact cumulative gas is `25163 + 315 = 25478`. -/
theorem validPositiveRound5Mix0DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round5Mix0Mem3 (round4DoneZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 25478 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix0DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have completed the first round-5
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-5 message arguments `m[2]` and `m[12]`.
Exact cumulative gas is `25190 + 315 = 25505`. -/
theorem validPositiveRound5Mix0DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix0Mem3 (round4DoneOneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 25505 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix0DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
