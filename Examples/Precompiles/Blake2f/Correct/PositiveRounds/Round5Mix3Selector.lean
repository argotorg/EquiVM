...
/-!
# BLAKE2F positive-round wrappers: round-5 fourth `mixG` argument setup

Context-level wrappers for the fourth round-5 `mixG` argument setup on the path with at least
six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have prepared the fourth round-5
`mixG` call.

The cursor is PC `2199`, with packed round-5 SIGMA word `0x2c6a0b834d75fe19` and message
arguments `m[8]`/`m[3]`. Exact cumulative gas is `26306 + 93 = 26399`. -/
theorem validPositiveRound5Mix3ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2199⟩
      [round5M3Arg (round5Mix2ZeroMem ctx.executionEnv),
        round5M8Arg (round5Mix2ZeroMem ctx.executionEnv),
        ⟨2383⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix2ZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 26399 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix3ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have prepared the fourth round-5
`mixG` call.

The cursor is PC `2199`, with packed round-5 SIGMA word `0x2c6a0b834d75fe19` and message
arguments `m[8]`/`m[3]`. Exact cumulative gas is `26333 + 93 = 26426`. -/
theorem validPositiveRound5Mix3ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2199⟩
      [round5M3Arg (round5Mix2OneMem ctx.executionEnv),
        round5M8Arg (round5Mix2OneMem ctx.executionEnv),
        ⟨2383⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix2OneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 26426 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix3ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
