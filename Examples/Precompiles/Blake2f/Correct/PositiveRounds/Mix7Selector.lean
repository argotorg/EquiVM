...
/-!
# BLAKE2F positive-round wrappers: eighth `mixG` argument setup

Context-level wrappers for the last diagonal round-0 `mixG` argument setup.
-/
...
/-- Valid final-flag-`0` inputs with positive rounds have prepared the last diagonal round-0
`mixG`.

The cursor is PC `3109`, immediately before the shared body that updates bytecode vector slots
`v[3]`, `v[4]`, `v[9]`, and `v[14]`. Exact cumulative gas is `10705`. -/
theorem validPositiveRoundMix7ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (seventhMixMem3
        (sixthMixMem3
          (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v13MixedMem ctx.executionEnv))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 10705 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  simpa [BytecodeContext.initialState] using validPositiveRoundMix7ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with positive rounds have prepared the last diagonal round-0
`mixG`.

The cursor is PC `3109`, preserving the previous vector updates. Exact cumulative gas is `10732`. -/
theorem validPositiveRoundMix7ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (seventhMixMem3
        (sixthMixMem3
          (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v14FinalFlagMem ctx.executionEnv))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 10732 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  simpa [BytecodeContext.initialState] using validPositiveRoundMix7ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
