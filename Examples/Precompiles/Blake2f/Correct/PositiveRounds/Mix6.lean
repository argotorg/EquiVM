...
/-!
# BLAKE2F positive-round wrappers: seventh `mixG`

Context-level wrappers for the completed third diagonal round-0 `mixG`.
-/
...
/-- Valid final-flag-`0` inputs with positive rounds have completed the third diagonal round-0
`mixG`.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]`. Exact cumulative gas is `10621`. -/
theorem validPositiveRoundMix6DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 10621 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  simpa [BytecodeContext.initialState] using validPositiveRoundMix6DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with positive rounds have completed the third diagonal round-0
`mixG`.

The cursor is PC `3070`, preserving the final-flag memory overwrite and first seven `mixG` updates.
Exact cumulative gas is `10648`. -/
theorem validPositiveRoundMix6DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 10648 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  simpa [BytecodeContext.initialState] using validPositiveRoundMix6DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
