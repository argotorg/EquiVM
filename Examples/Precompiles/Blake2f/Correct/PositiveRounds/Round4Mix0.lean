...
/-!
# BLAKE2F positive-round wrappers: round-4 first `mixG`

Context-level wrappers for the completed first round-4 `mixG` call on valid inputs with at least
five rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least five rounds have completed the first round-4
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-4 message arguments `m[9]` and `m[0]`.
Exact cumulative gas is `21652 + 315 = 21967`. -/
theorem validPositiveRound4Mix0DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound4Word, ⟨3109⟩, ⟨3292⟩, ⟨4⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round4Mix0Mem3 (round3DoneZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 21967 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix0DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least five rounds have completed the first round-4
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-4 message arguments `m[9]` and `m[0]`.
Exact cumulative gas is `21679 + 315 = 21994`. -/
theorem validPositiveRound4Mix0DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound4Word, ⟨3109⟩, ⟨3292⟩, ⟨4⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round4Mix0Mem3 (round3DoneOneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 21994 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix0DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
