...
/-!
# BLAKE2F positive-round wrappers: round-4 fourth diagonal `mixG`

Context-level wrappers for the completed fourth diagonal round-4 `mixG` call on the path with at
least five rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least five rounds have completed all eight round-4
`mixG` calls.

The cursor is PC `3292`, immediately after updating bytecode vector slots `v[3]`, `v[4]`,
`v[9]`, and `v[14]` with round-4 message arguments `m[3]` and `m[13]`.
Exact cumulative gas is `24529 + 318 = 24847`. -/
theorem validPositiveRound4Mix7DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3292⟩
      [⟨4⟩, ⟨1⟩, ⟨640⟩, UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩,
        ⟨384⟩, ⟨1472⟩, ⟨168⟩, ⟨1216⟩]
      (round4Mix7Mem3 (round4Mix6ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 24847 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix7DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least five rounds have completed all eight round-4
`mixG` calls.

The cursor is PC `3292`, immediately after updating bytecode vector slots `v[3]`, `v[4]`,
`v[9]`, and `v[14]` with round-4 message arguments `m[3]` and `m[13]`.
Exact cumulative gas is `24556 + 318 = 24874`. -/
theorem validPositiveRound4Mix7DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 24874 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix7DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
