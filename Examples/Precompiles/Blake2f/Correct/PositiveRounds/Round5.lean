...
/-!
# BLAKE2F positive-round wrappers: round-5 completion

Context-level wrappers after round 5 has completed and control has returned to the rounds-loop
guard with loop index `6`.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have completed round 5 and returned to
the rounds-loop guard.

The cursor is PC `1370` with loop index `6`. Exact cumulative gas is `28373`. -/
theorem validPositiveRound5DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28373 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have completed round 5 and returned to
the rounds-loop guard.

The cursor is PC `1370` with loop index `6`. Exact cumulative gas is `28400`. -/
theorem validPositiveRound5DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28400 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
