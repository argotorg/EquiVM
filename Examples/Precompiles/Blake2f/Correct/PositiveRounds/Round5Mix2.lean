...
/-!
# BLAKE2F positive-round wrappers: round-5 third `mixG`

Context-level wrappers for the completed third round-5 `mixG` call on valid inputs with at least
six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have completed the third round-5
`mixG` call.

The cursor is PC `2153`, immediately after updating bytecode vector slots `v[2]`, `v[6]`,
`v[10]`, and `v[14]` with round-5 message arguments `m[0]` and `m[11]`.
Exact cumulative gas is `25985 + 321 = 26306`. -/
theorem validPositiveRound5Mix2DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round5Mix2Mem3 (round5Mix1ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 26306 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix2DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have completed the third round-5
`mixG` call.

The cursor is PC `2153`, immediately after updating bytecode vector slots `v[2]`, `v[6]`,
`v[10]`, and `v[14]` with round-5 message arguments `m[0]` and `m[11]`.
Exact cumulative gas is `26012 + 321 = 26333`. -/
theorem validPositiveRound5Mix2DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix2Mem3 (round5Mix1OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 26333 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix2DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
