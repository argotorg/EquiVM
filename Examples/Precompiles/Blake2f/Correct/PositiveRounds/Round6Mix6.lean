...
/-!
# BLAKE2F positive-round wrappers: round-6 third diagonal `mixG`

Context-level wrappers for the completed third diagonal round-6 `mixG` call on the path with at
least seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have completed the third diagonal
round-6 `mixG` call.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]` with round-6 message arguments `m[9]` and `m[2]`.
Exact cumulative gas is `31168 + 321 = 31489`. -/
theorem validPositiveRound6Mix6DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round6Mix6Mem3 (round6Mix5ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31489 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix6DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have completed the third diagonal
round-6 `mixG` call.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]` with round-6 message arguments `m[9]` and `m[2]`.
Exact cumulative gas is `31195 + 321 = 31516`. -/
theorem validPositiveRound6Mix6DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix6Mem3 (round6Mix5OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31516 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix6DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
