...
/-!
# BLAKE2F positive-round wrappers: round-2 first `mixG`

Context-level wrappers for the completed first round-2 `mixG` call on the path with at least three
rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have completed the first round-2
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-2 message arguments `m[11]` and `m[8]`.
Exact cumulative gas is `14696 + 315 = 15011`. -/
theorem validPositiveRound2Mix0DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round2Mix0Mem3 (round1DoneZeroMem ctx.executionEnv))
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix0DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have completed the first round-2
`mixG` call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-2 message arguments `m[11]` and `m[8]`.
Exact cumulative gas is `14723 + 315 = 15038`. -/
theorem validPositiveRound2Mix0DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round2Mix0Mem3 (round1DoneOneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k
  ...
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix0DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
