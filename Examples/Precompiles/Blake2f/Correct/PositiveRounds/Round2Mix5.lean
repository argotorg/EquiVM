...
/-!
# BLAKE2F positive-round wrappers: round-2 second diagonal `mixG`

Context-level wrappers for the completed second diagonal round-2 `mixG` call on the path with at
least three rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have completed the second diagonal
round-2 `mixG` call.

The cursor is PC `2840`, immediately after updating bytecode vector slots `v[1]`, `v[6]`,
`v[11]`, and `v[12]` with round-2 message arguments `m[3]` and `m[6]`.
Exact cumulative gas is `16754 + 321 = 17075`. -/
theorem validPositiveRound2Mix5DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round2Mix5Mem3 (round2Mix4ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 17075 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix5DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have completed the second diagonal
round-2 `mixG` call.

The cursor is PC `2840`, immediately after updating bytecode vector slots `v[1]`, `v[6]`,
`v[11]`, and `v[12]` with round-2 message arguments `m[3]` and `m[6]`.
Exact cumulative gas is `16781 + 321 = 17102`. -/
theorem validPositiveRound2Mix5DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round2Mix5Mem3 (round2Mix4OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 17102 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix5DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
