...
/-!
# BLAKE2F positive-round wrappers: round-2 completion

Context-level wrappers after round 2 has completed and control has returned to the rounds-loop
guard with loop index `3`.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have completed round 2 and returned to
the rounds-loop guard.

The cursor is PC `1370` with loop index `3`. Exact cumulative gas is `17906`. -/
theorem validPositiveRound2DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1370⟩
      [⟨3⟩, ⟨640⟩, UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩,
        ⟨384⟩, ⟨1472⟩, ⟨168⟩, ⟨1216⟩]
      (round2Mix7Mem3 (round2Mix6ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 17906 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have completed round 2 and returned to
the rounds-loop guard.

The cursor is PC `1370` with loop index `3`. Exact cumulative gas is `17933`. -/
theorem validPositiveRound2DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 17933 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
