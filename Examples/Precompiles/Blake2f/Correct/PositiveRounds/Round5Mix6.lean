...
/-!
# BLAKE2F positive-round wrappers: round-5 third diagonal `mixG`

Context-level wrappers for the completed third diagonal round-5 `mixG` call on the path with at
least six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have completed the third diagonal
round-5 `mixG` call.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]` with round-5 message arguments `m[15]` and `m[14]`.
Exact cumulative gas is `27635 + 321 = 27956`. -/
theorem validPositiveRound5Mix6DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round5Mix6Mem3 (round5Mix5ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 27956 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix6DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have completed the third diagonal
round-5 `mixG` call.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]` with round-5 message arguments `m[15]` and `m[14]`.
Exact cumulative gas is `27662 + 321 = 27983`. -/
theorem validPositiveRound5Mix6DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix6Mem3 (round5Mix5OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 27983 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix6DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
