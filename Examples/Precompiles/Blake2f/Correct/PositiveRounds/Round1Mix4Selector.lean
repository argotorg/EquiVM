...
/-!
# BLAKE2F positive-round wrappers: round-1 first diagonal `mixG` argument setup

Context-level wrappers for the first diagonal round-1 `mixG` argument setup on the multi-round
path.
-/
...
/-- Valid final-flag-`0` inputs with at least two rounds have prepared the first diagonal
round-1 `mixG` call. Exact cumulative gas is `12901`. -/
theorem validPositiveRound1Mix4ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
            ...
            (round1Mix0Mem3
              (eighthMixMem3
                (seventhMixMem3
                  (sixthMixMem3
                    (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v13MixedMem ctx.executionEnv)))))))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 12901 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix4ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least two rounds have prepared the first diagonal
round-1 `mixG` call. Exact cumulative gas is `12928`. -/
theorem validPositiveRound1Mix4ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
            ...
            (round1Mix0Mem3
              (eighthMixMem3
                (seventhMixMem3
                  (sixthMixMem3
                    (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v14FinalFlagMem ctx.executionEnv)))))))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 12928 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix4ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
