...
/-!
# BLAKE2F positive-round wrappers: round-1 fourth `mixG` argument setup

Context-level wrappers for the fourth round-1 `mixG` argument setup on the multi-round path.
-/
...
/-- Valid final-flag-`0` inputs with at least two rounds have prepared the fourth round-1 `mixG`
call. Exact cumulative gas is `12487`. -/
theorem validPositiveRound1Mix3ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
          ...
          (round1Mix0Mem3
            (eighthMixMem3
              (seventhMixMem3
                (sixthMixMem3
                  (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v13MixedMem ctx.executionEnv))))))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 12487 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix3ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least two rounds have prepared the fourth round-1 `mixG`
call. Exact cumulative gas is `12514`. -/
theorem validPositiveRound1Mix3ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
          ...
          (round1Mix0Mem3
            (eighthMixMem3
              (seventhMixMem3
                (sixthMixMem3
                  (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v14FinalFlagMem ctx.executionEnv))))))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 12514 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix3ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
