...
/-!
# BLAKE2F positive-round wrappers: round-4 first diagonal `mixG`

Context-level wrappers for the completed first diagonal round-4 `mixG` call on valid inputs with
...
/-- Valid final-flag-`0` inputs with at least five rounds have completed the first diagonal
round-4 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-4 message arguments `m[14]` and `m[1]`.
Exact cumulative gas is `23302 + 315 = 23617`. -/
theorem validPositiveRound4Mix4DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound4Word, ⟨3109⟩, ⟨3292⟩, ⟨4⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round4Mix4Mem3 (round4Mix3ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 23617 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix4DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least five rounds have completed the first diagonal
round-4 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-4 message arguments `m[14]` and `m[1]`.
Exact cumulative gas is `23329 + 315 = 23644`. -/
theorem validPositiveRound4Mix4DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound4Word, ⟨3109⟩, ⟨3292⟩, ⟨4⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round4Mix4Mem3 (round4Mix3OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 23644 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix4DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
