...
/-!
# BLAKE2F positive-round wrappers: round-6 second diagonal `mixG` argument setup

Context-level wrappers for the second diagonal round-6 `mixG` argument setup on the path with at
least seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have prepared the second diagonal
round-6 `mixG` call.

The cursor is PC `2656`, with packed round-6 SIGMA word `0xc5feda418b309762` and message
arguments `m[6]`/`m[3]`. Exact cumulative gas is `30661 + 93 = 30754`. -/
theorem validPositiveRound6Mix5ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2656⟩
      [round6M3Arg (round6Mix4ZeroMem ctx.executionEnv),
        round6M6Arg (round6Mix4ZeroMem ctx.executionEnv),
        ⟨2840⟩, sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix4ZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 30754 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix5ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have prepared the second diagonal
round-6 `mixG` call.

The cursor is PC `2656`, with packed round-6 SIGMA word `0xc5feda418b309762` and message
arguments `m[6]`/`m[3]`. Exact cumulative gas is `30688 + 93 = 30781`. -/
theorem validPositiveRound6Mix5ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2656⟩
      [round6M3Arg (round6Mix4OneMem ctx.executionEnv),
        round6M6Arg (round6Mix4OneMem ctx.executionEnv),
        ⟨2840⟩, sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix4OneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 30781 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix5ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
