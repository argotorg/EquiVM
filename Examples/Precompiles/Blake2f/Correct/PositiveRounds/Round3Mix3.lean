...
/-!
# BLAKE2F positive-round wrappers: round-3 fourth `mixG`

Context-level wrappers for the completed fourth round-3 `mixG` call on valid inputs with at least
four rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least four rounds have completed the fourth round-3
`mixG` call.

The cursor is PC `2383`, immediately after updating bytecode vector slots `v[3]`, `v[7]`,
`v[11]`, and `v[15]` with round-3 message arguments `m[11]` and `m[14]`.
Exact cumulative gas is `19399 + 321 = 19720`. -/
theorem validPositiveRound3Mix3DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round3Mix3Mem3 (round3Mix2ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 19720 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix3DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least four rounds have completed the fourth round-3
`mixG` call.

The cursor is PC `2383`, immediately after updating bytecode vector slots `v[3]`, `v[7]`,
`v[11]`, and `v[15]` with round-3 message arguments `m[11]` and `m[14]`.
Exact cumulative gas is `19426 + 321 = 19747`. -/
theorem validPositiveRound3Mix3DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round3Mix3Mem3 (round3Mix2OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 19747 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix3DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
