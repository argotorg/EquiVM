...
/-!
# BLAKE2F positive-round wrappers: round-6 second diagonal `mixG`

Context-level wrappers for the completed second diagonal round-6 `mixG` call on the path with at
least seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have completed the second diagonal
round-6 `mixG` call.

The cursor is PC `2840`, immediately after updating bytecode vector slots `v[1]`, `v[6]`,
`v[11]`, and `v[12]` with round-6 message arguments `m[6]` and `m[3]`.
Exact cumulative gas is `30754 + 321 = 31075`. -/
theorem validPositiveRound6Mix5DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round6Mix5Mem3 (round6Mix4ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31075 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix5DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have completed the second diagonal
round-6 `mixG` call.

The cursor is PC `2840`, immediately after updating bytecode vector slots `v[1]`, `v[6]`,
`v[11]`, and `v[12]` with round-6 message arguments `m[6]` and `m[3]`.
Exact cumulative gas is `30781 + 321 = 31102`. -/
theorem validPositiveRound6Mix5DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix5Mem3 (round6Mix4OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31102 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix5DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
