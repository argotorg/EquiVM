...
/-!
# BLAKE2F positive-round wrappers: round-3 third diagonal `mixG`

Context-level wrappers for the completed third diagonal round-3 `mixG` call on the path with at
least four rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least four rounds have completed the third diagonal
round-3 `mixG` call.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]` with round-3 message arguments `m[4]` and `m[0]`.
Exact cumulative gas is `20635 + 321 = 20956`. -/
theorem validPositiveRound3Mix6DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round3Mix6Mem3 (round3Mix5ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 20956 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix6DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least four rounds have completed the third diagonal
round-3 `mixG` call.

The cursor is PC `3070`, immediately after updating bytecode vector slots `v[2]`, `v[7]`,
`v[8]`, and `v[13]` with round-3 message arguments `m[4]` and `m[0]`.
Exact cumulative gas is `20662 + 321 = 20983`. -/
theorem validPositiveRound3Mix6DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round3Mix6Mem3 (round3Mix5OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 20983 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix6DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
