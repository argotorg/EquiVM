...
/-!
# BLAKE2F positive-round wrappers: round-4 second `mixG`

Context-level wrappers for the completed second round-4 `mixG` call on valid inputs with at least
five rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least five rounds have completed the second round-4
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-4 message arguments `m[5]` and `m[7]`.
Exact cumulative gas is `22060 + 321 = 22381`. -/
theorem validPositiveRound4Mix1DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound4Word, ⟨3109⟩, ⟨3292⟩, ⟨4⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round4Mix1Mem3 (round4Mix0ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 22381 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix1DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least five rounds have completed the second round-4
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-4 message arguments `m[5]` and `m[7]`.
Exact cumulative gas is `22087 + 321 = 22408`. -/
theorem validPositiveRound4Mix1DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound4Word, ⟨3109⟩, ⟨3292⟩, ⟨4⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round4Mix1Mem3 (round4Mix0OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 22408 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound4Mix1DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
