...
/-!
# BLAKE2F positive-round wrappers: round-2 second `mixG`

Context-level wrappers for the completed second round-2 `mixG` call on the path with at least
three rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have completed the second round-2
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-2 message arguments `m[12]` and `m[0]`.
Exact cumulative gas is `15104 + 321 = 15425`. -/
theorem validPositiveRound2Mix1DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round2Mix1Mem3 (round2Mix0ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 15425 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix1DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have completed the second round-2
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-2 message arguments `m[12]` and `m[0]`.
Exact cumulative gas is `15131 + 321 = 15452`. -/
theorem validPositiveRound2Mix1DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round2Mix1Mem3 (round2Mix0OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 15452 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix1DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
