...
/-!
# BLAKE2F positive-round wrappers: round-5 third diagonal `mixG` argument setup

Context-level wrappers for the third diagonal round-5 `mixG` argument setup on the path with at
least six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have prepared the third diagonal
round-5 `mixG` call.

The cursor is PC `2886`, with packed round-5 SIGMA word `0x2c6a0b834d75fe19` and message
arguments `m[15]`/`m[14]`. Exact cumulative gas is `27542 + 93 = 27635`. -/
theorem validPositiveRound5Mix6ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2886⟩
      [round5M14Arg (round5Mix5ZeroMem ctx.executionEnv),
        round5M15Arg (round5Mix5ZeroMem ctx.executionEnv),
        ⟨3070⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix5ZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 27635 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix6ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have prepared the third diagonal
round-5 `mixG` call.

The cursor is PC `2886`, with packed round-5 SIGMA word `0x2c6a0b834d75fe19` and message
arguments `m[15]`/`m[14]`. Exact cumulative gas is `27569 + 93 = 27662`. -/
theorem validPositiveRound5Mix6ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨2886⟩
      [round5M14Arg (round5Mix5OneMem ctx.executionEnv),
        round5M15Arg (round5Mix5OneMem ctx.executionEnv),
        ⟨3070⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix5OneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 27662 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix6ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
