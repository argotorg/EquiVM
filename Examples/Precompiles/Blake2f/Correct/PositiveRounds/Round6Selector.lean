...
/-!
# BLAKE2F positive-round wrappers: round-6 selector

Context-level wrappers for preparing the first round-6 `mixG` call on valid inputs with at least
seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have selected round-6 message
arguments `m[12]` and `m[5]`.

The cursor is PC `1512`, immediately before the shared `mixG` body. Exact cumulative gas is
`28696`. -/
theorem validPositiveRound6Mix0ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1512⟩
      [round6M5Arg (round5DoneZeroMem ctx.executionEnv),
        round6M12Arg (round5DoneZeroMem ctx.executionEnv),
        ⟨1693⟩, sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5DoneZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28696 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix0ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have selected round-6 message
arguments `m[12]` and `m[5]`.

The cursor is PC `1512`, immediately before the shared `mixG` body. Exact cumulative gas is
`28723`. -/
theorem validPositiveRound6Mix0ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1512⟩
      [round6M5Arg (round5DoneOneMem ctx.executionEnv),
        round6M12Arg (round5DoneOneMem ctx.executionEnv),
        ⟨1693⟩, sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5DoneOneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28723 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix0ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
