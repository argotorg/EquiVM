...
/-!
# BLAKE2F positive-round wrappers: round-5 selector

Context-level wrappers for preparing the first round-5 `mixG` call on valid inputs with at least
six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have selected round-5 message
arguments `m[2]` and `m[12]`.

The cursor is PC `1512`, immediately before the shared `mixG` body. Exact cumulative gas is
`25163`. -/
theorem validPositiveRound5Mix0ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1512⟩
      [round5M12Arg (round4DoneZeroMem ctx.executionEnv),
        round5M2Arg (round4DoneZeroMem ctx.executionEnv),
        ⟨1693⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round4DoneZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 25163 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix0ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have selected round-5 message
arguments `m[2]` and `m[12]`.

The cursor is PC `1512`, immediately before the shared `mixG` body. Exact cumulative gas is
`25190`. -/
theorem validPositiveRound5Mix0ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1512⟩
      [round5M12Arg (round4DoneOneMem ctx.executionEnv),
        round5M2Arg (round4DoneOneMem ctx.executionEnv),
        ⟨1693⟩, sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round4DoneOneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 25190 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix0ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
