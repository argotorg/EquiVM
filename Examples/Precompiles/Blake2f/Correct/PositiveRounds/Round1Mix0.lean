...
/-!
# BLAKE2F positive-round wrappers: round-1 first `mixG`

Context-level wrappers for the completed first round-1 `mixG` call on the multi-round path.
-/
...
/-- Valid final-flag-`0` inputs with at least two rounds have completed the first round-1 `mixG`
call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-1 message arguments `m[14]` and `m[10]`. Exact cumulative gas is `11566`. -/
theorem validPositiveRound1Mix0DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 11566 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix0DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least two rounds have completed the first round-1 `mixG`
call.

The cursor is PC `1693`, immediately after updating bytecode vector slots `v[0]`, `v[4]`, `v[8]`,
and `v[12]` with round-1 message arguments `m[14]` and `m[10]`. Exact cumulative gas is `11593`. -/
theorem validPositiveRound1Mix0DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 11593 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix0DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
