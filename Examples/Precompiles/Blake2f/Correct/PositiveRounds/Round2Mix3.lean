...
/-!
# BLAKE2F positive-round wrappers: round-2 fourth `mixG`

Context-level wrappers for the completed fourth round-2 `mixG` call on the path with at least
three rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have completed the fourth round-2
`mixG` call.

The cursor is PC `2383`, immediately after updating bytecode vector slots `v[3]`, `v[7]`,
`v[11]`, and `v[15]` with round-2 message arguments `m[15]` and `m[13]`.
Exact cumulative gas is `15932 + 321 = 16253`. -/
theorem validPositiveRound2Mix3DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round2Mix3Mem3 (round2Mix2ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 16253 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix3DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have completed the fourth round-2
`mixG` call.

The cursor is PC `2383`, immediately after updating bytecode vector slots `v[3]`, `v[7]`,
`v[11]`, and `v[15]` with round-2 message arguments `m[15]` and `m[13]`.
Exact cumulative gas is `15959 + 321 = 16280`. -/
theorem validPositiveRound2Mix3DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round2Mix3Mem3 (round2Mix2OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 16280 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix3DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
