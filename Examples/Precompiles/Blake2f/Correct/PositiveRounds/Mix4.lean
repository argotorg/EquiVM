...
/-!
# BLAKE2F positive-round wrappers: fifth `mixG`

Context-level wrappers for the completed first diagonal round-0 `mixG`.
-/
...
/-- Valid final-flag-`0` inputs with positive rounds have completed the first diagonal round-0
`mixG`.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]`. Exact cumulative gas is `9793`. -/
theorem validPositiveRoundMix4DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 9793 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  simpa [BytecodeContext.initialState] using validPositiveRoundMix4DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with positive rounds have completed the first diagonal round-0
`mixG`.

The cursor is PC `2610`, preserving the final-flag memory overwrite and first five `mixG` updates.
Exact cumulative gas is `9820`. -/
theorem validPositiveRoundMix4DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 9820 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  simpa [BytecodeContext.initialState] using validPositiveRoundMix4DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
