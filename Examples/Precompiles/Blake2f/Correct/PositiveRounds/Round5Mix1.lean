...
/-!
# BLAKE2F positive-round wrappers: round-5 second `mixG`

Context-level wrappers for the completed second round-5 `mixG` call on valid inputs with at least
six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have completed the second round-5
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-5 message arguments `m[6]` and `m[10]`.
Exact cumulative gas is `25571 + 321 = 25892`. -/
theorem validPositiveRound5Mix1DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round5Mix1Mem3 (round5Mix0ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 25892 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix1DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have completed the second round-5
`mixG` call.

The cursor is PC `1923`, immediately after updating bytecode vector slots `v[1]`, `v[5]`,
`v[9]`, and `v[13]` with round-5 message arguments `m[6]` and `m[10]`.
Exact cumulative gas is `25598 + 321 = 25919`. -/
theorem validPositiveRound5Mix1DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound5Word, ⟨3109⟩, ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix1Mem3 (round5Mix0OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 25919 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix1DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
