...
/-!
# BLAKE2F positive-round wrappers: round-6 fourth diagonal `mixG`

Context-level wrappers for the completed fourth diagonal round-6 `mixG` call on the path with at
least seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have completed all eight round-6
`mixG` calls.

The cursor is PC `3292`, immediately after updating bytecode vector slots `v[3]`, `v[4]`,
`v[9]`, and `v[14]` with round-6 message arguments `m[8]` and `m[11]`.
Exact cumulative gas is `31573 + 318 = 31891`. -/
theorem validPositiveRound6Mix7DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3292⟩
      [⟨6⟩, ⟨1⟩, ⟨640⟩, UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩,
        ⟨384⟩, ⟨1472⟩, ⟨168⟩, ⟨1216⟩]
      (round6Mix7Mem3 (round6Mix6ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31891 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix7DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have completed all eight round-6
`mixG` calls.

The cursor is PC `3292`, immediately after updating bytecode vector slots `v[3]`, `v[4]`,
`v[9]`, and `v[14]` with round-6 message arguments `m[8]` and `m[11]`.
Exact cumulative gas is `31600 + 318 = 31918`. -/
theorem validPositiveRound6Mix7DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [⟨6⟩, ⟨1⟩, ⟨640⟩, UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩,
        ⟨384⟩, ⟨1472⟩, ⟨168⟩, ⟨1216⟩]
      (round6Mix7Mem3 (round6Mix6OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31918 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix7DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
