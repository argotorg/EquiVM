...
/-!
# BLAKE2F positive-round wrappers: round-1 first diagonal `mixG`

Context-level wrappers for the completed first diagonal round-1 `mixG` call on the multi-round
path.
-/
...
/-- Valid final-flag-`0` inputs with at least two rounds have completed the first diagonal
round-1 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-1 message arguments `m[1]` and `m[12]`.
Exact cumulative gas is `13216`. -/
theorem validPositiveRound1Mix4DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 13216 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix4DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least two rounds have completed the first diagonal
round-1 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-1 message arguments `m[1]` and `m[12]`.
Exact cumulative gas is `13243`. -/
theorem validPositiveRound1Mix4DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 13243 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix4DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
