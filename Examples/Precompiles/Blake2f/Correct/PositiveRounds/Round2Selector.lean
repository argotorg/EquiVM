...
/-!
# BLAKE2F positive-round wrappers: round-2 SIGMA selector

Context-level wrappers for the first round-2 `mixG` argument setup on the path with at least
three rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have prepared the first round-2
`mixG` call.

The cursor is PC `1512`, with packed round-2 SIGMA word `0xb8c052fdae367194` and message
arguments `m[11]`/`m[8]`. Exact cumulative gas is `14696`. -/
theorem validPositiveRound2Mix0ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1512⟩
      [round2M8Arg (round1DoneZeroMem ctx.executionEnv),
        round2M11Arg (round1DoneZeroMem ctx.executionEnv),
        ⟨1693⟩, sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round1DoneZeroMem ctx.executionEnv)
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix0ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have prepared the first round-2
`mixG` call.

The cursor is PC `1512`, with packed round-2 SIGMA word `0xb8c052fdae367194` and message
arguments `m[11]`/`m[8]`. Exact cumulative gas is `14723`. -/
theorem validPositiveRound2Mix0ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨1512⟩
      [round2M8Arg (round1DoneOneMem ctx.executionEnv),
        round2M11Arg (round1DoneOneMem ctx.executionEnv),
        ⟨1693⟩, sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round1DoneOneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k
  ...
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix0ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
