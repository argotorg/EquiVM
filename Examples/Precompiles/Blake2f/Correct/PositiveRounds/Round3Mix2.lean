...
/-!
# BLAKE2F positive-round wrappers: round-3 third `mixG`

Context-level wrappers for the completed third round-3 `mixG` call on valid inputs with at least
four rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least four rounds have completed the third round-3
`mixG` call.

The cursor is PC `2153`, immediately after updating bytecode vector slots `v[2]`, `v[6]`,
`v[10]`, and `v[14]` with round-3 message arguments `m[13]` and `m[12]`.
Exact cumulative gas is `18985 + 321 = 19306`. -/
theorem validPositiveRound3Mix2DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round3Mix2Mem3 (round3Mix1ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 19306 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix2DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least four rounds have completed the third round-3
`mixG` call.

The cursor is PC `2153`, immediately after updating bytecode vector slots `v[2]`, `v[6]`,
`v[10]`, and `v[14]` with round-3 message arguments `m[13]` and `m[12]`.
Exact cumulative gas is `19012 + 321 = 19333`. -/
theorem validPositiveRound3Mix2DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound3Word, ⟨3109⟩, ⟨3292⟩, ⟨3⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round3Mix2Mem3 (round3Mix1OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 19333 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound3Mix2DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
