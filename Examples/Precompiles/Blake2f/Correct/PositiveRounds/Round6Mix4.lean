...
/-!
# BLAKE2F positive-round wrappers: round-6 first diagonal `mixG`

Context-level wrappers for the completed first diagonal round-6 `mixG` call on valid inputs with
at least seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have completed the first diagonal
round-6 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-6 message arguments `m[0]` and `m[7]`.
Exact cumulative gas is `30346 + 315 = 30661`. -/
theorem validPositiveRound6Mix4DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round6Mix4Mem3 (round6Mix3ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 30661 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix4DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have completed the first diagonal
round-6 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-6 message arguments `m[0]` and `m[7]`.
Exact cumulative gas is `30373 + 315 = 30688`. -/
theorem validPositiveRound6Mix4DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix4Mem3 (round6Mix3OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 30688 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix4DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
