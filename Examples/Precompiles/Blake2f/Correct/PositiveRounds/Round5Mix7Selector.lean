...
/-!
# BLAKE2F positive-round wrappers: round-5 fourth diagonal `mixG` argument setup

Context-level wrappers for the fourth diagonal round-5 `mixG` argument setup on the path with at
least six rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least six rounds have prepared the fourth diagonal
round-5 `mixG` call.

The cursor is PC `3109`, with message arguments `m[1]`/`m[9]`. Exact cumulative gas is
`27956 + 84 = 28040`. -/
theorem validPositiveRound5Mix7ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3109⟩
      [round5M9Arg (round5Mix6ZeroMem ctx.executionEnv),
        round5M1Arg (round5Mix6ZeroMem ctx.executionEnv),
        ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix6ZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28040 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix7ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least six rounds have prepared the fourth diagonal
round-5 `mixG` call.

The cursor is PC `3109`, with message arguments `m[1]`/`m[9]`. Exact cumulative gas is
`27983 + 84 = 28067`. -/
theorem validPositiveRound5Mix7ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3109⟩
      [round5M9Arg (round5Mix6OneMem ctx.executionEnv),
        round5M1Arg (round5Mix6OneMem ctx.executionEnv),
        ⟨3292⟩, ⟨5⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round5Mix6OneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 28067 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound5Mix7ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
