...
/-!
# BLAKE2F positive-round wrappers: round-6 fourth diagonal `mixG` argument setup

Context-level wrappers for the fourth diagonal round-6 `mixG` argument setup on the path with at
least seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have prepared the fourth diagonal
round-6 `mixG` call.

The cursor is PC `3109`, with message arguments `m[8]`/`m[11]`. Exact cumulative gas is
`31489 + 84 = 31573`. -/
theorem validPositiveRound6Mix7ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3109⟩
      [round6M11Arg (round6Mix6ZeroMem ctx.executionEnv),
        round6M8Arg (round6Mix6ZeroMem ctx.executionEnv),
        ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix6ZeroMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31573 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix7ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have prepared the fourth diagonal
round-6 `mixG` call.

The cursor is PC `3109`, with message arguments `m[8]`/`m[11]`. Exact cumulative gas is
`31516 + 84 = 31600`. -/
theorem validPositiveRound6Mix7ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ⟨3109⟩
      [round6M11Arg (round6Mix6OneMem ctx.executionEnv),
        round6M8Arg (round6Mix6OneMem ctx.executionEnv),
        ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix6OneMem ctx.executionEnv)
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 31600 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix7ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
