...
/-!
# BLAKE2F positive-round wrappers: round-1 SIGMA selector

Context-level wrappers for the first round-1 `mixG` argument setup on the multi-round path.
-/
...
/-- Valid final-flag-`0` inputs with at least two rounds have prepared the first round-1 `mixG`
call.

The cursor is PC `1512`, with packed round-1 SIGMA word `0xea489fd61c02b753` and message arguments
`m[14]`/`m[10]`. Exact cumulative gas is `11251`. -/
theorem validPositiveRound1Mix0ArgsZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (eighthMixMem3
        (seventhMixMem3
          (sixthMixMem3
            (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v13MixedMem ctx.executionEnv)))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 11251 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix0ArgsZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least two rounds have prepared the first round-1 `mixG`
call.

The cursor is PC `1512`, with packed round-1 SIGMA word `0xea489fd61c02b753` and message arguments
`m[14]`/`m[10]`. Exact cumulative gas is `11278`. -/
theorem validPositiveRound1Mix0ArgsOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      (eighthMixMem3
        (seventhMixMem3
          (sixthMixMem3
            (fifthMixMem3 (fourthMixMem3 (thirdMixMem3 (secondMixMem3 (firstMixMem3 (v14FinalFlagMem ctx.executionEnv)))))))))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 11278 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound1Mix0ArgsOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
