...
/-!
# BLAKE2F positive-round wrappers: round-6 third `mixG`

Context-level wrappers for the completed third round-6 `mixG` call on valid inputs with at least
seven rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least seven rounds have completed the third round-6
`mixG` call.

The cursor is PC `2153`, immediately after updating bytecode vector slots `v[2]`, `v[6]`,
`v[10]`, and `v[14]` with round-6 message arguments `m[14]` and `m[13]`.
Exact cumulative gas is `29518 + 321 = 29839`. -/
theorem validPositiveRound6Mix2DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round6Mix2Mem3 (round6Mix1ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 29839 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix2DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least seven rounds have completed the third round-6
`mixG` call.

The cursor is PC `2153`, immediately after updating bytecode vector slots `v[2]`, `v[6]`,
`v[10]`, and `v[14]` with round-6 message arguments `m[14]` and `m[13]`.
Exact cumulative gas is `29545 + 321 = 29866`. -/
theorem validPositiveRound6Mix2DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound6Word, ⟨3109⟩, ⟨3292⟩, ⟨6⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round6Mix2Mem3 (round6Mix1OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 29866 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound6Mix2DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
