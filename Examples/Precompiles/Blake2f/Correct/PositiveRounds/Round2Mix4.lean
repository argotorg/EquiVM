...
/-!
# BLAKE2F positive-round wrappers: round-2 first diagonal `mixG`

Context-level wrappers for the completed first diagonal round-2 `mixG` call on the path with at
least three rounds.
-/
...
/-- Valid final-flag-`0` inputs with at least three rounds have completed the first diagonal
round-2 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-2 message arguments `m[10]` and `m[14]`.
Exact cumulative gas is `16346 + 315 = 16661`. -/
theorem validPositiveRound2Mix4DoneZeroFlagPrefix
    (ctx : BytecodeContext)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        ...
        ⟨168⟩, ⟨1216⟩]
      (round2Mix4Mem3 (round2Mix3ZeroMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 16661 := by
  ...
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix4DoneZeroFlagPrefixGas
...
/-- Valid final-flag-`1` inputs with at least three rounds have completed the first diagonal
round-2 `mixG` call.

The cursor is PC `2610`, immediately after updating bytecode vector slots `v[0]`, `v[5]`,
`v[10]`, and `v[15]` with round-2 message arguments `m[10]` and `m[14]`.
Exact cumulative gas is `16373 + 315 = 16688`. -/
theorem validPositiveRound2Mix4DoneOneFlagPrefix
    (ctx : BytecodeContext)
    (hcode : ctx.executionEnv.code = runtimeBytecode)
    (haccepts : accepts ctx)
    (hvalid : valid ctx)
    ...
    ∃ k, RDx runtimeBytecode ctx.executionEnv ctx.gas ctx.initialState
      ...
      [sigmaRound2Word, ⟨3109⟩, ⟨3292⟩, ⟨2⟩, ⟨1⟩, ⟨640⟩,
        UInt256.shiftRight (inputFirstWord ctx.executionEnv) ⟨224⟩, ⟨384⟩, ⟨1472⟩,
        ⟨168⟩, ⟨1216⟩]
      (round2Mix4Mem3 (round2Mix3OneMem ctx.executionEnv))
      (UInt256.ofNat 62) ByteArray.empty (ctx.createdAccounts, ctx.accountMap) k 16688 := by
  obtain ⟨hwv, _hsize⟩ := haccepts
  obtain ⟨hlen, _hflag⟩ := hvalid
  have hlen' : ctx.executionEnv.calldata.size = 213 := by
    simpa [Model.inputLength] using hlen
  simpa [BytecodeContext.initialState] using validPositiveRound2Mix4DoneOneFlagPrefixGas
    (cA := ctx.createdAccounts)
    (gh := ctx.genesisBlockHeader)
    (bl := ctx.blocks)
    (σ := ctx.accountMap)
