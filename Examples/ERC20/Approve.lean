...
theorem erc20ApproveX_dec1852_spender {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
    (hsz68 : 68 ≤ I.calldata.size) (hsize : I.calldata.size < UInt256.size)
    (hszhi : I.calldata.size < 2 ^ 255 + 4)
    (hreach : ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨100⟩ [sel]
      solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C) :
    ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨1852⟩
        [approveSpenderWord I, ⟨1888⟩, approveSpenderWord I, ⟨4⟩ + ⟨0⟩,
...
theorem erc20ApproveX_dec1835_spender {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
    (hsz68 : 68 ≤ I.calldata.size) (hsize : I.calldata.size < UInt256.size)
    (hszhi : I.calldata.size < 2 ^ 255 + 4)
    (hreach : ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨100⟩ [sel]
      solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C) :
    ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨1835⟩
        [approveSpenderWord I, ⟨1861⟩, approveSpenderWord I, ⟨1888⟩,
          approveSpenderWord I, ⟨4⟩ + ⟨0⟩, UInt256.ofNat I.calldata.size, ⟨1980⟩,
          ⟨0⟩, ⟨0⟩, ⟨0⟩, ⟨4⟩, UInt256.ofNat I.calldata.size, ⟨121⟩, ⟨126⟩, sel]
        solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C := by
  obtain ⟨k, C, rd⟩ := erc20ApproveX_dec1852_spender (cA := cA) (gh := gh) (bl := bl)
    (σ := σ) (σ₀ := σ₀) (A := A) (g := g) (sel := sel) hsz68 hsize hszhi hreach
  exact ⟨_, _, evm_run rd with [
    jumpdest, push2 ⟨1861⟩, dup2, push2 ⟨1835⟩, jump erc20_jd ]⟩

theorem erc20ApproveX_dec1861_spender {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
    (hsz68 : 68 ≤ I.calldata.size) (hsize : I.calldata.size < UInt256.size)
    (hszhi : I.calldata.size < 2 ^ 255 + 4)
    (hreach : ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨100⟩ [sel]
      solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C) :
    ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨1861⟩
        [UInt256.land (approveSpenderWord I) erc20AddrMask, approveSpenderWord I, ⟨1888⟩,
          approveSpenderWord I, ⟨4⟩ + ⟨0⟩, UInt256.ofNat I.calldata.size, ⟨1980⟩,
          ⟨0⟩, ⟨0⟩, ⟨0⟩, ⟨4⟩, UInt256.ofNat I.calldata.size, ⟨121⟩, ⟨126⟩, sel]
        solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C := by
  obtain ⟨k, C, rd⟩ := erc20ApproveX_dec1835_spender (cA := cA) (gh := gh) (bl := bl)
    (σ := σ) (σ₀ := σ₀) (A := A) (g := g) (sel := sel) hsz68 hsize hszhi hreach
  exact ⟨_, _, evm_run rd with [
...
theorem erc20ApproveX_dec1980 {cA gh bl σ σ₀ A I} {g : Sat256} {sel : UInt256}
    ...
    ∃ k C, RD erc20Bytecode I g (initState cA gh bl σ σ₀ g A I) ⟨1980⟩
        [approveSpenderWord I, ⟨0⟩, ⟨0⟩, ⟨0⟩, ⟨4⟩, UInt256.ofNat I.calldata.size,
          ⟨121⟩, ⟨126⟩, sel]
        solcFreePtrMem (UInt256.ofNat 3) ByteArray.empty (cA, σ) k C := by
  have hclean : UInt256.eq (approveSpenderWord I)
      (UInt256.land (approveSpenderWord I) erc20AddrMask) = ⟨1⟩ :=
    erc20Canon_eq hcanonSpender
  obtain ⟨k, C, rd⟩ := erc20ApproveX_dec1861_spender (cA := cA) (gh := gh) (bl := bl)
    (σ := σ) (σ₀ := σ₀) (A := A) (g := g) (sel := sel) hsz68 hsize hszhi hreach
  exact ⟨_, _, evm_run rd with [
