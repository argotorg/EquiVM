...
theorem evalExpr_transfer_debit (evm : EVM.State) (I : ExecutionEnv)
  ...
  have hdiffNonneg :
      0 ≤ Int.ofNat (transferFromBalanceWord evm).toNat -
        Int.ofNat (transferValueWord I).toNat := by
    rw [hsub]
  ...
  have hdiffFit :
    ...
    rw [hsub]
    have hfit :
        (transferFromBalanceWord evm).toNat - (transferValueWord I).toNat <
          UInt256.size :=
      lt_of_le_of_lt (Nat.sub_le _ _) (transferFromBalanceWord evm).val.isLt
  ...
  have hsubEval :
      evalIntArithResult (.uint ⟨256, by decide⟩) .checked
        ...
        .ok (.int
          (Int.ofNat (transferFromBalanceWord evm).toNat -
            Int.ofNat (transferValueWord I).toNat)) :=
    evalIntArithResult_checked_uint_ok _ _ hdiffNonneg hdiffFit
  ...
  simp only [evalExpr?, EvalResult.ofOption, transferStoreFromBalance_fromBalance,
    transferStoreFromBalance_value, EvalResult.bind, bind]
  simp only [evalBinaryOp?, transferFromBalanceValue, transferValueValue]
  ...
  have haddEval :
      evalIntArithResult (.uint ⟨256, by decide⟩) .checked
        ...
        .ok (.int
          (Int.ofNat (transferToBalanceWord evm I).toNat +
            Int.ofNat (transferValueWord I).toNat)) :=
    evalIntArithResult_checked_uint_ok _ _ hsumNonneg hsumFit
  have hbinaryEval :
      evalExpr? erc20Config
        { contract := erc20Contract, locals := transferStoreToBalance evm I }
        (transferAfterDebitState evm I)
    ...
    simp only [evalExpr?, EvalResult.ofOption, transferStoreToBalance_toBalance,
      transferStoreToBalance_value, EvalResult.bind, bind]
    exact haddEval
...
theorem erc20TransferBodyReturns (evm : EVM.State) (I : ExecutionEnv)
  ...
  refine ExecBlock.consNormal (ExecStmt.letDecl (evalExpr_transfer_sender_balance evm I)
    (valueMatchesOptionalABIType_uint256_word (transferFromBalanceWord evm))) ?_
  ...
  refine ExecBlock.consNormal (ExecStmt.letDecl (evalExpr_transfer_newToBalance evm I hfit)
