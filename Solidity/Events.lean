...
/-- The log entry for `emit ev(args)` (arguments already in the `Solm.Value` domain). -/
def mkLogEntry (this : EVM.Address) (ev : EventInfo) (args : List Solm.Value) :
